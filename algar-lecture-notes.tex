...
\defequiv (\varphi \Rightarrow \bot)$, verify intuitionistically:
\begin{enumerate}
\item $\varphi \Rightarrow \neg\neg\varphi$
\item $\neg\neg\neg\varphi \Leftrightarrow \neg\varphi$
\item $\neg\neg(\varphi \vee \neg\varphi)$
\item The principle of excluded middle, for all statements~$\varphi$, is
equivalent to the principle of double negation elimination for all statements.

{\noindent\scriptsize\emph{Hint.}
Don't try to verify that double negation elimination for a specific
statement~$\varphi$ implies the principle of excluded middle for that same
statement~$\varphi$ -- this cannot be shown.\par}
\end{enumerate}
\end{exercise}
...
\label{ex:diaconescu}
The axiom of choice can be put as: ``Every surjective map~$f$ has a
section~$s$, that is a map in the other direction such that~$f \circ s =
\mathrm{id}$.'' This exercise is on Diaconescu's theorem, stating that
