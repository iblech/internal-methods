...
There is also a somewhat less used, but philosophically rewarding and more
...
answer is~$k[U]$, where~$k$ is some base field.) However, if we calculate the
...
of~$U^2$ in~$\AA^1_X$. In contrast, the sets~$\{ x\?\affl \,|\, x \neq 0 \}$
...
Part~\ref{part:big-zariski}. % repetition: "deal"
...
% This has been mentioned before: in the remark above; in the paragraph after
% the definition of local ring.
...
\begin{multline*}
  ...
  % Only the first of the three is needed!
  % All three together mean equality of the radical ideals, not inclusion.
...
% (This process halts iff the chain is not strictly ascending.)
...
% This uses that (every ascending chain stabilizes) iff
% (there is no strictly ascending chain).
...
generally \emph{sublocales} of~$X$).
...
% Why is there no \Box in front of the equality?
\end{align*}
\end{ex}
...
full story at the moment. See the IHÉS 2015 slides.}
...
\XXX{This lattice is indeed always a frame, \ie this definition always gives rise to a
locale.}
