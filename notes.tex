...
% XXX: Also elimination of prime ideals.
...
This lemma in particular implies that constant sheaves enjoy several
...
internal/external reasoning with the generic filter.
...
is an element of every prime ideal. This might be calculationally simpler.
...
a proof we can't extract any upper bound on~$n$.
...
\begin{prop}Let~$M$ and~$N$ be~$A$-modules. Let~$\alpha : M \to N$ be a ring
homomorphism. The interpretations of the statements in the second column of
...
\begin{proof}To demonstrate the technique we verify the second and last claim.
...
be done, using the frame of radical ideals in~$A$, but we won't discuss details
here.
% XXX: give reference
...
of~$X$, defined on varying stages, as a single set.
...
sense that nonzero elements are invertible.\footnote{The proof goes as follows.
...
flavor, in that one can show that from the internal point of view \emph{any
map~$\affl \to \affl$ is given by a (unique) polynomial with coefficients
in~$\affl$}. This is related to the fact that the value of the internal
Hom~$[\affl,\affl]$ at an~$S$-scheme~$T$ is given
by~$\Gamma(\AA^1_T,\O_{\AA^1_T})$.\footnote{In synthetic differential geometry,
there is a similar proposition: From the internal point of view, any map~$\RR
\to \RR$ is smooth (in an appropriate sense).}

With~$\affl$ at hand, we can perform many of the usual constructions of scheme
theory internally.

\begin{itemize}
\item The functors associated to the standard group schemes~$\GG_\text{a}$, $\GG_\text{m}$,
...
\end{align*}
\item Affine~$n$-space over~$S$ is given by~$(\affl)^n$, \ie internally the set
...
communication).
\item More generally, for an~$S$-scheme~$X$, affine and projective~$n$-space
over~$X$ are given by~$\ul{X} \times (\affl^1)^n$ and~$\ul{X} \times \PP^n$,
respectively.
\item Let~$X$ be an~$S$-scheme and~$f \in \O_X(X)$ be a global function. Since
...
respectively~$V(f)$.\footnote{From the internal point of view,
...
general.}
\item In particular, the set~$\Delta \defeq \{ \varepsilon\?\affl \,|\,
...
\ZZ[\varepsilon]$, so explicitly it has the same underlying topological space
as~$S$, but~$\O_S[\varepsilon]/(\varepsilon^2)$ instead of~$\O_S$ as structure
sheaf.
\item For an~$S$-scheme~$X$, the internal Hom~$[\Delta,\ul{X}]$ describes the
``tangent bundle'' of~$X$, \ie the
scheme~$\RelSpec{X}{\operatorname{Sym}(\Omega^1_{X/S})}$, as can be seen by
...
specify first-order information. This description of the (not necessarily
...
\item Let~$\E$ be an~$\affl$-algebra. Then~$[\E,\affl]_\affl$, \ie internally the set
...
If the functor~$\E$ is given
by~$\E(T \xra{\mu} S) \defeq \Gamma(T, \mu^*\A)$, where~$\A$ is a
quasicoherent~$\O_S$-algebra, the functor~$[\E,\affl]_\affl$ is precisely the
functor of points of the~$S$-scheme~$\RelSpec{S}{\A}$.

\item Conversely, if~$X \xra{\mu} S$ is an~$S$-scheme, the internal
...
\RelSpec{S}(\mu_*\O_X)$ is an isomorphism. We can reformulate this condition in
...
topology, the topos of sheaves on~$\Sch/S$ with respect to~$\tau$ is a subtopos
...
\varphi$'' means that~$\varphi$ holds~$\tau$-locally.

A basic example is provided by the Kummer sequence, that is the short sequence
...
\begin{question}Is there an internal characterization of the modal
operator~$\Box_\text{ét}$? For instance, is~$\Box_\text{ét}$ the smallest modal
operator~$\Box$ such that~$\affl$ is a~$\Box$-sheaf and such that monic
separable polynomials of positive degree~$\Box$-possess a simple root? An
article of Wraith~\cite{wraith:generic-galois-theory} is related to this
question, but falls just short of answering it.
\end{question}
...
\item modal operators for finer topologies
\end{itemize}
...
\begin{longtable}{@{}p{4.4cm}@{\qquad}p{6.7cm}@{\qquad}p{1.5cm}@{}}
  ...
  $X$ is quasicompact & ``$\Sh(X) \models$'' commutes with directed disjuctions & Prop.\@~\ref{prop:quasicompact-meta} \\
  $X$ is local & ``$\Sh(X) \models$'' commutes with arbitrary disjuctions & Prop.\@~\ref{prop:local-meta} \\
  $X$ is irreducible & if $\neg(\varphi \wedge \psi)$, then $\neg\varphi$ or~$\neg\psi$ & Prop.\@~\ref{prop:irreducibility-internally} \\
  \bottomrule
\end{longtable}}
% missing: relative spectrum
...
How can we prove that Reg(O_X)(U) = Reg O_X(U) for U affine?
...
A[S_U^(-1)] otherwise?
Relevant: lemma-directed-colimits-sections in stacks-project/sites.
