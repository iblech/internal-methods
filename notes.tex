...
opens is the lattice of radical ideals of~$A$.\end{defn}
...
\begin{prop}Let~$A$ be a ring. Then the points of the locale~$\Spec(A)$ are in
...
\[ F \defeq \{ x \? A \,|\, \sqrt{(x)} \in K \} \subseteq A, \]
...
showing that the construction doesn't have the expected universal property and
...
call ``filters''. Joyal's example was none other than~$\affl$
