...
$\dim X \leq 0$. The converse holds if~$X$ is integral or locally Noetherian.
\end{cor}
\begin{proof}
...
The following technical lemma will occasionally be handy. We will generalize it
to schemes which are not necessarily integral or reduced in
...
We will generalize this lemma to non-integral schemes below (see
Lemma~\ref{lemma:torsion-module-generic-stalk-generalized}).
...
\[ \Sh(X) \models \neg\neg\varphi \Longrightarrow \exists f\?\O_X\_
  \neg\neg(\speak{$f$ \inv}) \wedge (\speak{$f$ \inv} \Rightarrow \varphi) \]
respectively
\[ \Sh(X) \models \Box\varphi \Longrightarrow \exists f\?\O_X\_
  \Box(\speak{$f$ \inv}) \wedge (\speak{$f$ \inv} \Rightarrow \varphi). \]
\end{lemma}
...
Let~$X$ be an integral or locally Noetherian scheme of dimension~$\leq 0$. Then
