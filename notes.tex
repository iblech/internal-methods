...
\newcommand{\RelSpec}[2]{\mathrm{RelSpec}_{#1}{#2}}  % XXX: phase out
...
\tocless\subsection*{Internal language of toposes}
...
\tocless\subsection*{Algebraic geometry}
...
\tocless\subsection*{Limitations} The internal language is \emph{local}, in the sense
...
\tocless\subsection*{Introductory literature} These notes are intended
...
\tocless\subsection*{Related work} The internal language of toposes was applied to algebraic geometry before. For
...
\tocless\subsection*{Notational convention} Occasionally, when quantifying, we use
...
\chapter{The little Zariski topos}
...
\tocless\subsection*{Naive approach}
...
\tocless\subsection*{Using Grothendieck universes}
...
\tocless\subsection*{Approach of the Stacks Project}
...
\tocless\subsection*{Employing parsimonious sites}
...
the 1970s by Anders Kock~\cite{kock:univ-proj-geometry}.
...
% Proof of third and eigth property references first property by number.
\begin{enumerate}
...
It's worthwhile to introduce the notion of an \emph{open truth value}. This
files our approach to synthetic algebraic geometry in the established and more
general framework of synthetic topology~\cite{XXX}:
...
that~$\affl/J$ is quasicoherent and such that~$U$ holds if and only if~$1 \in
J$. (Section~\ref{sect:basics-on-truth-values} contains generalities on truth
values.)\end{defn}

\begin{prop}Let~$X$ be a Zariski sheaf. A subfunctor~$U \hookrightarrow X$ is
open if and only if, from the internal point of view of~$\Zar(S)$, for
any~$x\?X$ the truth value of~``$x \in U$'' is an open truth value.\end{prop}
\begin{proof}Immediate from the definition and the characterization given in
Proposition~\ref{prop:characterization-open-subfunctor}.\end{proof}
...
\subseteq \affl$ such that~$\affl/J$ is quasicoherent and such that~$U$ holds
if and only if~$1 \in J$. By assumption, the element~1 is \notnot an element
of~$J$; we want to verify that it's actually an element of~$J$.

By Proposition~\ref{prop:XXX}, the canonical homomorphism
\[ \affl/J \longrightarrow [\Spec(\affl/J), \affl] \]
is bijective; the assumptions of that proposition are satisfied in either of
...
\section{Case study: Grassmannian}
...
\begin{defn}A ring~$R$ is \emph{weakly Noetherian} if and only if any ideal
...
\begin{thm}Let~$R$ be a weakly Noetherian ring. Then the polynomial
algebra~$R[X]$ is weakly Noetherian as well, even intuitionistically.
\end{thm}
...
\begin{prop}Let~$X$ be an arbitrary reduced scheme (not necessarily locally
...
% XXX: Show Grothendieck's generic freeness
