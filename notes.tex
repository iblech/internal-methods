...
fixed point. Let~$\varphi(\J)$ be a statement on radical ideals~$\J$ such that
$\varphi(\sup_i \J_i) \Leftrightarrow \bigvee_i \varphi(\J_i)$ for arbitrary
families~$(\J_i)_i$ of radical ideals. If
...
In classical logic, where complementation yields an one-to-one correspondence
...
Before giving the proof, we want to give clarify some details of the
construction.
...
locales internal to~$\Sh(X)$, or expressed externally: The relative
...
correspondence with those filters~$F \subseteq A$ with
\[ \varphi(r) \in F \Longrightarrow \speak{$r$ \inv\@ in $R$} \]
for all~$r \? R$.
\end{prop}
...
\begin{align*}
  \aaa &\defeq \sup \{ \sqrt{(1)} \,|\, \text{$r$ is invertible} \} \\
  &\phantom{\vcentcolon}= \{ s\?A \,|\, \text{$s$ is nilpotent or $r$ is invertible} \}
  \in \Open(\Spec(A)).
\end{align*}
...
\begin{lemma}Let~$R$ be a local ring. Let~$\varphi : R \to A$ be an~$R$-algebra.
...
\[ \Spec(A|R) \models
  \forall f\?\ul{R}\_
  \speak{$\varphi(f)$ is \inv\@ in~$\O_{\Spec(A|R)}$} \Rightarrow
...
the internal spectrum (instead of the internal local spectrum) if and
