...
local sections carry rings structures and these ring strictures are compatible
...
fact of linear algebra that if in a short exact sequence of modules the two
outer ones are finitely generated the middle one is too. The usual proof of
this fact is intuitionistically acceptable and can thus be interpreted in the
internal language. It then \emph{automatically} yields the more advanced
proposition that if in a short exact sequence of sheaves of~$\O_X$-modules the
two outer ones are of finite type the middle one is too.
...
the statement that any finitely generated vector space is free is stronger that
the double negated version and can not be shown. It would imply that any sheaf
...
\[ \renewcommand{\arraystretch}{1.3}\begin{array}{@{}lcl@{}}
  ...
  U \models \exists s \? \F\_ \varphi(s) &\Ll&
    \hcancel{\text{there exists a section~$s \in \Gamma(U,\F)$ s.\,th. $U
    \models \varphi(s)$}}{0pt}{3pt}{0pt}{-2pt} \\
  &&
    \text{there exists an open covering $U = \bigcup_i U_i$ s.\,th. for all~$i$:} \\
  && \quad\quad \text{there exists~$s_i \in \Gamma(U_i, \F)$ s.\,th.
...
Let~$x \in X$ be a point. Let~$\varphi$ be a geometric formula.
...
% comment that this lemma is very useful
...
% XXX: Explain that a sheaf of rings really does satisfy the ring axioms from
...
\subsection{Truth at stalks vs. truth on neighbourhoods}
