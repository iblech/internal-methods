...
The internal language of a scheme~$X$ is Boolean if and only if~$X$ has
dimension~$\leq 0$.
\end{prop}
...
The following technical lemma will occasionally be handy.
\begin{lemma}\label{lemma:repr-formula}
Let~$X$ be a scheme. Let~$\varphi$ be a formula over an open subset~$U$.
Then
\[ U \models \exists f\?\O_X\_ (\varphi \Leftrightarrow \speak{$f$ \inv}). \]
\end{lemma}
\begin{proof}
We may assume that~$U = \Spec A$ is an affine scheme. Then~$\brak{\varphi}$,
the largest open subset of~$U$ on which~$\varphi$ holds, is a union of
standard open subsets~$D(f_i)$. Since~$V \models \speak{$f_i$ \inv}$ if and
only if~$V \models D(f_i)$, we can take these~$f_i$ as witnesses of the
existential statement. \XXX{wrong}
\end{proof}
...
Let~$X$ be a scheme. Then $\dim X \leq 0$ if and only if the internal language
of~$\Sh(X)$ is Boolean.
\end{cor}
\begin{proof}
For the ``if'' direction, note that internally, any element~$s\?\O_X$ is
invertible or not invertible. In the latter case~$s$ is nilpotent, by
Proposition~\ref{prop:neginvnilpotent}. Thus the claim follows by the previous
corollary.

For the ``only if'' direction, it suffices to verify that internally, any
formula~$\varphi$ holds or does not hold. (The other classicality axiom, the
principle of double negation elimination, is equivalent to this one.) By
Lemma~\ref{lemma:repr-formula}, there exists an element~$s\?\O_X$ such
that~$\varphi$ holds if and only if~$s$ is invertible. By assumption,~$s$ is
invertible or nilpotent. In the first case,~$\varphi$ holds. In the second
case,~$\neg\varphi$ holds, since if~$s$ were invertible, we would have~$1 = 0
\? \O_X$.
\end{proof}
...
associated sublocale. (Co-)restricting the pushforward functor~$i_* : \Sh(X_\Box) \to
...
\end{quote}
Both conditions are satisfied if~$A$ is an integral domain or if~$A$ is
...
ideals. There are only a finite number of these prim ideals. Therefore the
...
subsetswhich have property~$(\star)$. For instance, this condition is satisfied
...
By Lemma~\ref{lemma:regular-affine}, the function~$f^n$ is also regular as an
element of~$\O_U$ from the internal point of view. Note that~$f^n$ is invertible
on~$V$, since~$V = D(f) \subseteq D(f^n)$. It follows that on the dense
open subset~$V \subseteq U$, the sections~$s$ and~$(f^n)^{-1} a$ agree.
This observation concludes the proof.
\end{proof}

\begin{cor}Let~$X$ be a reduced scheme having a cover by affine open subschemes
...
operator defined by
\[ \Box\varphi \defequiv \speak{$\O_X$ is~$(\varphi \Rightarrow
\placeholder)$-separated} \]
in the internal language of~$\Sh(X)$, \ie
\[ \Box\varphi \defequiv \forall s\?\O_X\_ (\varphi \Rightarrow s = 0)
\Rightarrow s = 0. \]

\begin{prop}Let~$U$ be an open subset of a scheme~$X$. Then~$\Sh(X) \models
\Box U$ if and only if~$U$ is scheme-theoretically dense in~$X$.
\end{prop}
...
property~$(\star\star)$, then $\K_X$ is the~$\Box$-sheafification of~$\O_X$.
\end{enumerate}
\end{prop}
...
notation, we may now suppose that~$V$ is a standard open subset such that~$U \models \Box
V$ (previously, we supposed that~$U \models \neg\neg V$). The proof that the
denominator~$f^n$ is regular (as seen from the internal perspective, as an
element of~$\O_U$) now goes as follows: We have~$V \subseteq
D(f^n)$. Therefore~$U \models \Box V$ implies~$U \models \Box(\speak{$f^n$ \inv})$. By
the fourth property, it follows that~$U \models \speak{$f^n$ is regular}$.
\end{proof}
...
\begin{prop}Let~$X$ be a scheme. Then~$\Box\varphi \Rightarrow \neg\neg\varphi$
for any formula~$\varphi$. The converse holds if~$X$ is reduced, such that in
this case the modal operator~$\Box$ coincides with the double negation modality.\end{prop}
\begin{proof}Let~$\varphi$ be an arbitrary formula. Note that~$\neg\varphi$ is
...
\models \Box U$ and is dense if and only if~$\Sh(X) \models \neg\neg U$.
Therefore the claim follows from the previous proposition.
\end{proof}

\begin{prop}Let~$X$ be a scheme admitting a cover of open affine subsets with
...
the sublocale~$X_\Box$. This result holds needs a cover with
...
\[ \F \defeq \{ M \in \Omega \,|\, \bigvee_{i \in I} (M = U_i) \}. \]
Recall that~$\Omega$ is the object of truth values, internally defined as the
power set of~$1 \defeq \{\star\}$ and that~``$M = U_i$'' is abuse of notation
