...
\times_{\Spec\ZZ} \ZZ[t]$. Its value on a~$S$-scheme~$T$ is given by
...
relations, and considering sets of maps between given objects. Additionally,
...
internal point of view. This example was suggested to us by Zhen~Lin Low.
...
if and only if there is a~$\tau$-covering~$(T_i \to T)_i$ of the~$S$-scheme~$T$
such that~$T_i \models \varphi$ for all~$i$. Intuitively, the formula
``$\Box_\tau \varphi$'' means that~$\varphi$ holds~$\tau$-locally.
% Kummer sequence
