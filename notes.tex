...
% XXX: Do they have a restriction on the characteristic?
...
\XXX{mention and explain: Mulvey/Burden, Vickers, Awodey, Coquand, ...; also
Martin Brandenburg (internalizing in a different sense)}
...
so-called geometric formulas play a
...
global functions on~$X$. Let~$\varphi \defequiv \neg(\speak{$f$ \inv})$
...
nucleus~$j_\Box$ associated to the modal operator~$\Box$. The former is given
by
\begin{align*}
  j_{\Ass(\O_X)}(U) &= \bigcup\ \{ V \subseteq X \ |\
  \text{$V$ open},\ V \cap \Ass(\O_X) \subseteq U \}, \\
\intertext{since \XXX{reason}, and recall from
...
\begin{align*}
  ...
  \Longleftrightarrow&\
    \Ass(\O_V) \subseteq U \\
  \Longleftrightarrow&\
    \text{the canonical morphism~$\O_V \to j_* \O_{U \cap V}$
    (with $j : U \cap V \hookrightarrow V$) is injective} \\
