...
slice topos~$\E/X \to \E$ will look like~$\Set/X$ from the internal point of
view of~$\E$.
...
the algebra~$\pi^{-1}\O_S \to \affl$. Indeed, the ring~$\affl$ is local and
the homomorphism~$\pi^{-1}\O_S \to \affl$ is local, since its restriction to
any sheaf topos~$\Sh(X)$, where~$f : X \to S$ is an~$S$-scheme contained in the site used
to define~$\Zar(S)$, is local: It's the canonical morphism $f^{-1}\O_S \to \O_X$.
