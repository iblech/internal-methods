...
\begin{prop}Let~$X$ be a reduced scheme. Then:
\begin{enumerate}
\item $\O_X$ is~$\neg\neg$-separated.
...
for~$X$ to be a ringed space such that the displayed equivalence above holds and
...
element of~$\O_X$ from the internal point of view. It follows that on the dense
open subset~$D(b) \cap V \subseteq U$, the sections~$s$ and~$b^{-1} a$ agree.
...
construction.
\end{proof}
% XXX: split this proof into parts, and maybe reorder qcoh before this
