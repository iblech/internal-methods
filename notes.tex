...
Constructing the spectrum as a locale instead of as a topological space
...
that therefore a true pointless approach as provided by locale theory, topos
theory, or formal topology is necessary to construct the spectrum in an
...
Reasoning fast and loose as follows could lead one to believe that it's
...
\begin{proof}The ordinary spectrum is always quasicompact. The local spectrum,
...
inclusion~$\RR \to \CC$ is local, but~$\CC \otimes_\RR \CC \cong \CC \times
\CC$ is not.
...
(constructed internal to~$\Sh(X)$). However, since the embedding
is not full, it's in general not the case that limits in~$\LRL$ are calculated by
applying the coreflector to the limit calculated in~$\RL$. Employing the language
of the proof of Proposition~\ref{prop:lrl-complete}, this construction only
...
seem awkward for a topos-theorist when taken out of context, since
...
that~$\Sh(S)$ classifies local localizations of~$A$ and~$\Zar(S)$ classifies
...
For instance, the trivial~$\E$-topos~$\E \to \E$ will look like~$\Set$ and the
slice topos~$\E/A \to \E$ will look like~$\Set/A$ from the internal point of
view of~$\E$.

\begin{thm}\label{thm:zar-classifies}
From the internal point of view of~$\Sh(S)$, the big Zariski
topos~$\Zar(S)$ is the classifying topos of the theory of local~$\O_S$-algebras
which are local over~$\O_S$.\end{thm}
...
endomorphisms satisfying some axioms. However, for a~$\Sh(S)$-topos~$f : \F \to
...
However, there is an internal geometric theory of~$\O_S$-algebras -- internal
...
\XXX{write down rest of the proof}
\end{proof}

% A corollary of the existence of~$\pi^!$ is that~$\pi_*$ is (not only
% continuous, but also) cocontinuous.

\XXX{Explain how to reconstruct~$\Sh(S)$ from~$\Zar(S)$}
