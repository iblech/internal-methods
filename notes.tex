...
\subsection{The universal filter}
...
In classical logic, the complement of a prime ideal defines a filter and
furthermore any filter is of such a form. In constructive mathematics however,
...
the localization of a ring at a prime ideal might not be a local.

\begin{defn}The \emph{universal filter}~$\F$ is the subsheaf of~$\ul{A}$
on~$\Spec A$ given by
...
\item From the internal point of view of~$\Sh(\Spec A)$, the universal
...
The significance of the universal filter is given by the following proposition.
\begin{prop}From the internal point of view of~$\Sh(\Spec A)$, the structure
sheaf~$\O_X$ is the localization of the constant sheaf~$\ul{A}$ away from the
universal filter.\end{prop}
...
\end{prop}
A simple syntactical criterion for the hypothesis on~$\I$ is that~$\I$ is
...
\begin{proof}Let~$D(f) \models \speak{$\I \cap \F$ is inhabited}$. Then there
...
0} (f^n \in \I)) \]
does not hold. Indeed, \XXX{present counterexample}.\end{rem}
