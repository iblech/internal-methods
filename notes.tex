...
\item $st \in F \dashv\vdash s \in F \wedge t \in F$ (one axiom for each $s,t\?A$)
...
morphism~$\flat\affl \to \affl$ to~$\E$ is an isomorphism. In the case that~$S
= \Spec(A)$ is affine, we also have the ring~$\ul{A}$ in~$\Zar(S)$ available.
In this case the condition is equivalent to
\[ \E \models \speak{$\ul{A} \to \affl$ is a localization}, \]
...
The objects of~$\Zar(S)$ listed on page~\ref{page:important-objects} pull back
as expected:
\begin{itemize}
...
just sets from the internal point of view of~$\Zar(S)$''.

A consequence for the internal language is that if~$\varphi$ is a formula
over~$\Zar(X)$,
\[
  \Zar(X) \models \varphi
    \qquad\text{iff}\qquad
