...
numerator~$s$ belongs to~$\S$, i.\,e. is an regular element of~$\O_X$.\end{rem}
...
from~$\O_X$ at regular elements, it holds that~$a/s = 0$ in~$\K_X$ if and only
if~$a = 0$ in~$\O_X$. Thus it follows that~$\neg\neg(a = 0)$ in~$\O_X$ and
thus~$a = 0$ in~$\O_X$; in particular, $a/s = 0$ in~$\K_X$.
...
representative~$f$, this~$f$ is well-defined up to multiplication by an element
of~$\O_X^*$.\end{defn}
...
\begin{defn}The \emph{Cartier divisor associated to a rank-one submodule}~$\L \subseteq
\K_X$ is~$D := [f^{-1}]$, where~$f\?\K_X$ is the unique element of some basis
of~$\L$.\end{defn}
