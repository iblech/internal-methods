...
statements without additional work.
...
For the second, recall from lemma~\ref{lemma:negneg-generic-point} that the
...
of~$\O_X$; specifically, it is the sheafification with respect to the model
operator defined by
...
language of~$\Sh(X)$ fulfills the following metaproperty: For any
formulas~$\varphi$ and~$\psi$
\[ \Sh(X) \models \neg(\varphi \wedge \psi)
  \quad\text{implies}\quad
  \Sh(X) \models \neg\varphi \text{ or }
  \Sh(X) \models \neg\psi. \]
Furthermore, in this case the following internal property holds:
...
Therefore, if~$X$ is irreducible, the claimed metaproperty holds. The converse
...
The stated internal property holds since open subsets of irreducible spaces are
irreducible.
\end{proof}
...
radical ideals as the lattice opens. This construction fulfills the expected
...
defined spectrum fulfills the universal property
\begin{align*}
...
to~$\Sh(X)$. However, the relative spectrum fulfills the different universal property
\begin{align*}
...
ascending process with values in~$M$ stops.
\end{defn}

\begin{defn}A ring~$R$ is \emph{constructively Noetherian} if and only if the
...
constructively Noetherian if and only if it is Noetherian in the usual sense.

The notion of a constructively Noetherian ring works well in an
intuitionistic context: Important rings such as~$\ZZ$ and more generally~$\O_K$
for any algebraic number field~$K$ are constructively Noetherian, and matrices
over Bézout rings which are integral domains in the weak sense and
constructively Noetherian can be put into Smith canonical form.

\begin{prop}A scheme~$X$ is locally Noetherian if and only if the ring~$\O_X$
is constructively Noetherian from the internal point of view.\end{prop}
