...
property: For any open subset~$U \subseteq X$, any sheaf of rings~$\R$ and any
homomorphism~$\O_X \to \R$ which maps all elements of~$\Gamma(V,\S)$, $V
\subseteq U$ to units, there exists exactly one homomorphism~$\O_X[\S^{-1}] \to
...
The ``$\mathrm{lc}$'' indicates that when interpreting this internal statement with the
