...
is turned into absolute scheme theory, but it still requires the machinery of
locally ringed spaces.

The internal language of the \emph{big} Zariski topos allows for a more far-reaching
change of perspective. It incorporates Grothendieck's functor-of-points
philosophy in order to cast modern algebraic geometry, relative to an arbitrary
base scheme, in a naive \emph{synthetic} language reminiscient of the classical
Italian school.
...
as closed subschemes of projective spaces, are in this way encoded using a huge
amount of machinery.
...
solutions for any ring. This approach requires fewer preparations and involves
only objects of intrinsic interest in algebraic geometry: $A$-valued points,
...
the approach using locally ringed spaces actually parameterize the irreducible
closed subsets.

However, the description of basic objects can still be somewhat involved. For
instance, while the functor for giving projective~$n$-space is given on fields
by the simple expression
\[ \begin{array}{r@{}c@{}l}
  k &{}\longmapsto{}& \text{the set of lines in~$k^{n+1}$} \\
  && \qquad \cong \{ [x_0:\ldots:x_n] \,|\, \text{$x_i \neq 0$ for some~$i$} \},
\end{array} \]
on general rings it is given by
\[
  A \longmapsto \text{the set of quotients~$A^n \twoheadrightarrow P$ modulo
  isomorphism}.
\]

On the one hand, typically only~$k$-valued points admit a simple description.
On the other hand, the $A$-valued points for more general rings~$A$ are crucial
in order to impart a meaningful sense of cohesion on the~$k$-valued
...
be given by polynomials. However, if we calculate the set of morphisms
in~$[\mathrm{Field},\Set]$ we obtain XXX.}
...
\[ \text{the set of lines in~$(\ul{\AA}^1)^{n+1}$}
  \quad\text{or}\quad
  \{ [x_0:\ldots:x_n] \,|\, \text{$x_i \neq 0$ for some~$i$} \}. \]
...
as the big étale topos, in Part~\ref{part:big-zariski}. The key point is
