...
ring is usually defined as the supremum of the lengths of all strictly
ascending prime ideals. Like with the classical definition of a local ring,
...
\begin{prop}Let~$R$ be a ring. Classically, it holds that~$R$ is constructively
of Krull dimension~$\leq n$ if and only if the usually defined Krull dimension
of~$R$ is less than or equal to~$n$.\end{prop}
...
\[ \dim X \leq n \quad\Longleftrightarrow\quad
  ...
  \]
\end{prop}
\begin{proof}A condition of the form~``$\sqrt{(f)} \subseteq \sqrt{(g,h)}$''
...
equivalent to the dimension of~$X$ being less than or equal to~$n$.
\end{proof}

\begin{cor}Let~$X$ be a scheme. Then:
...
The ``only if'' direction always holds. If any element of~$A$ is nilpotent or
invertible, the ``if'' direction holds as well (for any~$\aaa$ and any~$x$).
...
to be the empty scheme. Therefore the lemma is indeed applicable.) Since this
condition is a geometric implication, it is fulfilled in the internal language
if and only if it is fulfilled at every point~$x \in X$. This amounts to
requiring that in any local ring~$\O_{X,x}$, the unique maximal ideal is the
only prime ideal, \ie that all local rings~$\O_{X,x}$ have Krull dimension
zero. This is equivalent to~$X$ being empty or having dimension zero.
\end{proof}
