...
\begin{align*}
  ...
  \Longleftrightarrow\
  & X \models \forall s\?\F_ \forall t\?\F\_ \alpha(s) = \alpha(t) \Rightarrow s = t \\[0.5em]
  \Longleftrightarrow\ &
...
be a short exact sequence of~$\O_X$-modules. If~$\F$ and~$\H$ are locally
finitely free, so is~$\G$.
\end{lemma}
\begin{proof}From the internal perspective, we are given a short exact sequence
of modules with the outer ones being finitely free and we have to show that the
middle one is finitely free as well. It is well-known that this follows; and
since the usual proof of this fact is intuitionistic, we are done.
\end{proof}
...
basis vectors corresponding to the zero columns.
\end{proof}
...
This follows from the proof of the previous lemma, since it follows that the
kernel of the matrix is finitely free of rank zero.
\end{proof}
