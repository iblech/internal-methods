...
\item $f(x_0)$ is inhabited.
\item For any~$x_1 \in f(x_0)$, $f(x_1)$ is inhabited.
\item For any~$x_1 \in f(x_0)$ and any~$x_2 \in f(x_1)$, $f(x_2)$ is inhabited.
\item And so on.
\end{itemize}
Such a process \emph{halts} if and only if there
exists a step~$n$ and elements~$x_0 = x, x_1, \ldots, x_n$ such that~$x_{i+1}
...
definition. Mention equivalence with Richman's ascending tree condition.}
