...
Let~$\E$ be a sheaf of sets on~$S$. For reasons explained in
Section~\ref{sect:big-small-relation}, we denote by~``$\pi^{-1}(\E)$''
the induced sheaf on~$\Sch/S$ mapping an~$S$-scheme
$(f : T \to S)$ to~$\Gamma(T, f^{-1}(\E))$.

Let~$\E$ be a sheaf of~$\O_S$-modules. We denote by~``$\E^\Zar$''
the induced sheaf on~$\Sch/S$ mapping an~$S$-scheme
$(f : T \to S)$ to~$\Gamma(T, f^*(\E))$.
