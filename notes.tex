...
This observation is attributed by Tierney to Mulvey~\cite[p.~209]{tierney:spectrum}.
Tierney also states that ``its precise significance is still somewhat
obscure'' (ibid). % We think that it is significant because of ...
...
with~$\Box\varphi \defeqv (\speak{$f$ \inv} \Rightarrow \varphi)$.
...
\Gamma(T, \mu^*\A)$. The functor~$[\E,\affl]_\affl$ is precisely the
