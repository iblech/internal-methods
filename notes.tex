...
\begin{lemma}Let~$A$ be a ring. Then~$A$ is reduced if and only if all
stalks~$A_\ppp$, $\ppp \in \Spec A$, are reduced.\end{lemma}
\begin{proof}As discussed in Section~\ref{sect:reducedness}, all
stalks~$A_\ppp$ are reduced if and only if~$\O_{\Spec A}$ is a reduced ring
...
% XXX: Remark about uses in constructive mathematics
