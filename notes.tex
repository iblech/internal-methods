...
Grothendieck's generic freeness lemma in the following general form without any
Noetherian assumptions.
...
\begin{enumerate}
\item $M^\sim$ is locally free as an~$\O_{\Spec(A)}$-module,
\item $B^\sim$ is finitely presented as an~$\O_{\Spec(A)}$-algebra, and
\item $M^\sim$ is of finite presentation over~$B^\sim$.
\end{enumerate}
\end{thm}
