...
The structure sheaf~$\O_X$ of a scheme~$X$ is a sheaf of rings, that is the sets of
...
set~$\ZZ[t]$, as one would expect: Algebraic functions~$\AA^1 \to \AA^1$ should
be given by polynomials. (The discussion could also be relativized so that the
answer is~$k[t]$, where~$k$ is some base field.) However, if we calculate the
...
\ZZ[t]$. Its value on an~$X$-scheme~$T$ is
\[ \afflx(T) = \Hom_X(T, \AA^1_X) \cong \Hom_{\Spec\ZZ}(T, \Spec\ZZ[t]) \cong
\Gamma(T,\O_T). \]
...
field in the sense that nonzero elements are invertible. In the case~$S =
\Spec\ZZ$, this was first observed by Kock~\cite{kock:univ-proj-geometry}. At
the same time, it is not a reduced ring -- a feat only possible in an
...
ring~$\affl$ enjoys additional properties when studied in the étale topos, for
...
The standard reference for intuitionistic algebra is a textbook by Mines,
Richman and
...
The set~$M$ satisfies the \emph{ascending process condition} if and only if any
...
and~$\varphi(x) \defeq \{ y \,|\, \exists n\_ x = a_n \wedge y = a_{n+1} \}$.
...
\XXX{Rewrite the rest of this section to conform to the new (corrected)
definition.}
...
% XXX: Why restrict to connected open subsets?
...
\}. \]
% XXX: Wrong, \overline{I} as defined is not an ideal.
\end{enumerate}
\end{prop}
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[scale=1.4]
    ...
    \end{scope}
  \end{tikzpicture}
  \caption{\label{fig:iteration}An illustration of the iterative process used
  in the proof of Theorem~\ref{thm:generic-freeness}. The hatched cells
  ...
  combination of the vectors with smaller index (blue cells). It is therefore
...
elements are zero, minimal generating families are already linear independent;
...
In the first case, we remove the redundant vector~$x_1^{i_1} \cdots x_n^{i_n}
...
Figure~\ref{fig:iteration} illustrates this process. It's obvious that it
terminates after finitely many steps; this is related to the fact that
the ordinal~$\omega^n$ is well-founded.
\end{proof}
...
In classical logic, this form implies the lemma in its more abstract
...
in the future~\ref{blechschmidt:generic-freeness}. A part of the proof was
included by Brandenburg in a paper of his~\ref{brandenburg:schur}
...
\XXX{This section will be revised in the near future, it doesn't contain the
full story at the moment. See the IHES 2015 slides.}
...
morphism~$\RelSpec_X{\A} \to X$. We can give a simple construction of the
...
% XXX: Clean up the statement. Don't put the nontriviality assumption
% as a global condition. Also check whether we need to check that the map
...
\begin{align*}
  ...
  \Hom_{\LRL_{\Sh(X)}/(\pt,\O_X)}(I(Y), I(\RelSpec_X{\A})) \\
  & \cong \Hom_{\O_X}(\A, \mu_* \O_Y)
\end{align*}
...
\XXX{This lattice is always a frame, \ie this definition always gives rise to a
locale.}
...
spectrum~$\RelSpec_{\O_X}{\A}$.
\end{enumerate}
\end{prop}
\begin{proof}We first show that the internalization~$I(\RelSpec_{\O_X}{\A})$
...
\begin{align*}
  \Gamma(U, I(\RelSpec_{\O_X}{\A})) &\cong
  \Open(\RelSpec_{\O_X}{(\A)} \times_X U) \\
  & \cong
  \Open(\RelSpec_{\O_X|_U}{\A|_U}) \\
  & \cong
  \Open(\Spec S) \\
  & \cong
...
(as the pushforward under~$\RelSpec_{\O_X}{\A} \to X$ of a locale), and its
externalization coincides with~$\RelSpec_{\O_X}{\A}$.
\end{proof}
...
\chapter{The big Zariski topos}

\label{part:big-zariski}
...
(Definition~\ref{defn:kripke-joyal}) on are straightforward. Instead of
...
\intertext{they have to read as follows.}
  ...
  X \models \forall s\?\F\_ \varphi(s) \quad&\Ll\quad
  \text{for all morphisms~$T \to X$ in~$\Sch/S$ and} \\
  &\phantom{{}\Ll\quad\text{for }} \text{all sections~$s \in \Gamma(T, \F)$:} \\
  &\qquad\qquad\qquad\qquad V \models \varphi(s)
\end{align*}

\subsection*{Important objects in the big Zariski topos} It's convenient to
introduce notation for XXX objects which often appear when studying the big
Zariski topos.

\begin{itemize}
\item Let~$X$ be an~$S$-scheme. Its functor of points $(\Sch/S)^\op
\to \Set$, which maps an~$S$-scheme~$T$ to~$\Hom_S(T,X)$, is an object
of~$\Zar(S)$. We denote it by~``$\ul{X}$''.

Particularly important is~$\affl$, the functor of points of the affine line
over~$S$.

\item Let~$\E$ be a sheaf of sets on~$S$. For reasons explained in
Section~\ref{sect:big-small-relation}, we denote by~``$\pi^{-1}(\E)$''
the induced sheaf on~$\Sch/S$ given by
\[ (T \xra{f} S) \longmapsto \Gamma(T, f^{-1}(\E)). \]

\item Let~$\E$ be a sheaf of~$\O_S$-modules. We denote by~``$\E^\Zar$''
the induced sheaf on~$\Sch/S$ given by
\[ (T \xra{f} S) \longmapsto \Gamma(T, f^*(\E)). \]
\end{itemize}
...
\subsection{Quasicoherent sheaves of modules}
...
intuitionistic context. The following proposition compiles a short list of such
properties. At least one of these, the field property, was already noticed in
the 1970s by Kock~\cite{kock:univ-proj-geometry}.

\begin{prop}\label{prop:special-properties-a1}
The following properties of~$\affl$, as seen from the internal
...
% Proof of third and eighth property references first property by number.
\begin{enumerate}
...
\item An element of~$\affl$ is nilpotent if and only if it is \notnot zero.
...
\item Follows from the first and third property and the observation that
nilpotent elements are not invertible (else~$1 = 0$ in contradiction to~$\affl$
being a local ring).
...
Also the fifth statement cannot be satisfied in classical logic: for infinite
...
of~$\Zar(S)$, the map~$U \hookrightarrow X$ is a synthetic closed immersion.
In formal language:
\begin{multline*}
  \qquad\Zar(S) \models
  \forall x\?X\_
...
\sqrt{(0)}$ by Proposition~\ref{prop:special-properties-a1}. The
...
language of~$\Zar(S)$, is the functor of points of the punctured plane
...
Cite: Lombardi/Quitté, Commutative Algebra: Constructive Methods: Finite
Projective Modules.
