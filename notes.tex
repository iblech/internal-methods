...
modifications of Table~\ref{table:kripke-joyal} are straightforward. For
...
The big Zariski topos is the proper context for realizing the ``functor of
...
fulfills the gluing condition and is therefore an object of~$\Zar(S)$. From the
internal point of view of~$\Zar(S)$, the functor~$\ul{X}$ looks like a single
...
subjects, such as synthetic differential geometry, synthetic domain theory, or
synthetic computability theory, the internal language lets us pretend that our
objects of study, schemes over a base scheme, are plain sets and that
morphisms between our objects are simply maps between those sets.
% XXX: give references
% The additional structure ... is taken care of ...
...
respectively~$V(f)$.\footnote{Note that, from the internal point of view,
...
sheaf. For an~$S$-scheme~$X$ the internal Hom~$[\Delta,\ul{X}]$ describes the
``tangent bundle'' of~$X$, \ie the
scheme~$\RelSpec{S}{\operatorname{Sym}(\Omega^1_{X/S})}$. This fits nicely with
the intuition of tangent vectors as infinitesimal curves, and in fact is
precisely the definition of the tangent bundle in synthetic differential
geometry.
\end{itemize}
