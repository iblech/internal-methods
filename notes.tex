...
since the notions of a presheaf and a sheaf only need opens and coverings.
% XXX: give introductory reference

The lattice of opens of~$X_j$ is given by
...
Thus we can define the pullback of a sheaf on~$X$ and the pushforward of a
sheaf on~$X_j$ as usual.
\begin{align*}
  i^{-1} \F &= \text{sheafification of $(U \mapsto \colim_{U \preceq i^{-1}V} \Gamma(V,\F))$} \\
  i_* \G &= (U \mapsto \Gamma(i^{-1}U, \G) = \Gamma(j(U), \G)) \\
\end{align*}

% XXX: next step: describe internal sheaves and equivalence Sh_Box(Sh X) = Sh(X_Box).
