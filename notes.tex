...
The restriction to intuitionistic reasoning is not as worse as it might first
appear. We will discuss the practical consequences below (on
...
This example was not special: \emph{Any (intuitionistically acceptable) theorem
...
quasicompact'' or ``having vanishing sheaf cohomology'' can \emph{not} be
...
% mention and explain: Mulvey/Burden, Vickers, Awodey, Coquand, Caramello, ...
...
applications
\item note that parts of constructive mathematics get new fields of application
\end{itemize}
...
will see many examples of classically equivalent internal statements whose
...
finer distictions -- distictions which are not visible in classical logic.
...
\XXX{XXX: Is it clear that any constructively valid statement yields a valid sheaf
statement?}
...
We gloss over several details here. See~\cite[chapter~D4.1]{johnstone:elephant} for
a proper treatment.

To be able to fully express all constructions of ``usual mathematics'' in the
internal language (\ie those not specifically dependent
of the ambient logical framework), we need \emph{dependent types}.
\XXX{describe those}
...
a~$\mathrm{T}_1$-space~$X$ is \emph{Boolean} if and only if~$X$ is discrete,
and the internal language of a scheme~$X$ is Boolean if and only if~$X$ has
dimension~$\leq 0$.

However, there is also a more satisfactory answer, which furthermore
illuminates how to intuitively picture intuitionistic mathematics.
Namely, when doing intuitionistic mathematics, we use the same formal symbols, but with
...
do \emph{not} reject those axioms in the sense of postulating their
...
condition} this it involves quantification over subsets. It is also not of a
...
schemes, not shared with general locally ringed spaces. It is also a specific
...
assumption that some further element~$f\?\O_X$ is invertible. This is
...
\subsection{Krull dimension} Recall that the \emph{Krull dimension} of a
ring is usually defined as the supremum of the lengths of strictly
ascending chains of prime ideals. Like with the classical definition of a local ring,
...
was found by \XXX{authors} and simplified by Thierry Coquand \XXX{reference}.
...
\begin{itemize}
\item $\sqrt{(1)} \subseteq \sqrt{(a_0,b_0)}$
\item $\sqrt{(a_0 b_0)} \subseteq \sqrt{(a_1,b_1)}$
\item $\sqrt{(a_1 b_1)} \subseteq \sqrt{(a_2,b_2)}$
\item $\vdots$
\item $\sqrt{(a_{n-1} b_{n-1})} \subseteq \sqrt{(a_n,b_n)}$
\item $\sqrt{(a_n b_n)} = \sqrt{(0)}$
\end{itemize}
The ring~$R$ is \emph{constructively of Krull dimension~$\leq n$} if
...
constructively of Krull dimension~$\leq (-1)$.)
\end{defn}
...
For the following, no intuition about this definition is needed; however, we
...
For the special case~$n = 0$, the condition that~$R$ is constructively of Krull
...
\begin{enumerate}
\item In classical logic, the ring~$R$ is constructively
of Krull dimension~$\leq n$ if and only if the usually defined Krull dimension
of~$R$ is less than or equal to~$n$.
\item If the ring~$R$ is constructively
...
\end{enumerate}
\end{thm}
\begin{proof}\XXX{Give reference.}\end{proof}
...
\[ \dim X \leq n \quad\Longleftrightarrow\quad
  \Sh(X) \models \speak{$\O_X$ is constructively of Krull dimension~$\leq n$}
  \]
\end{prop}
\begin{proof}
...
Therefore the condition~$\speak{$\O_X$ is constructively of Krull
...
field in the (stronger) sense that any element of~$\O_X$ is invertible or zero.
\XXX{short explanation}
\end{cor}
...
language of~$\Sh(X)$: A local ring~$R$ is constructively of Krull
...
\varphi$. This mode of reasoning is called \emph{exhibiting a Brouwerian
...
\begin{multline*}
  ...
  (\exists u\?\O_X\_ g = uf) \qquad\qquad\qquad\qquad \qedhere
\end{multline*}
\end{proof}
...
fashion, we arrive at a diagonal matrix. Its diagonal entries already fulfill
...
Those phrases assume that one may do\XXX{word} the case distinction that for
any two ring elements~$x,y$, either~$x$ divides~$y$ or not. Fortunately, those
case distinctions are in fact superfluous.

A consequence of the corollary is that internal to the sheaf topos of a
Dedekind scheme, the usual structure theorem on finitely
presented~$\O_X$-modules is available. We will exploit this in \XXX{ref}.
...
the external notion of locally finite freeness follows because the
...
\subseteq M. \]
This definition is meaningful even if~$R$ is not an integral domain (in the
weak sense). An~$R$-module~$M$ is \emph{torsion-free} if and only if~$M_\tors$ is
...
\[ \exists a\?\O_X\_ (\forall b\?\O_X\_ ab = 0 \Rightarrow b = 0) \wedge
  as = 0. \]
...
\XXX{discuss alternative proofs}
...
\begin{lemma}Let~$X$ be a scheme (or ringed space). Let~$\F$ be
an~$\O_X$-module. Then:
\begin{enumerate}
...
For statement~(4), we have to prove that~$R$-modules of the form~$R^n$ are
...
to the zero module. In any case,~$R/(a_i)$ is finitely free and therefore~$M$
is finitely free as well.
\end{proof}
...
\ie the given number~$n$ such that~$n \in U$. Details omitted, since we will
...
trivially can deduce~$m \geq n+1$ as well. \qedhere
\end{proof}

If we want to work with a complete set of natural numbers in intuitionistic
logic, we have to construction a suitable completion.
...
One may restrict to open \emph{connected} neighbourhoods of~$X$, since any
...
over all natural numbers~$n$ such that~$\O_X$ is constructively of Krull
...
\begin{align*}
  ...
  & \qquad\qquad U \models \speak{$\O_X$ is constructively of Krull dimension~$\leq n$} \\
  \Longleftrightarrow &\
  \text{for some open connected neighbourhood~$U$ of~$x$,} \\
...
\begin{align*}
  \inf\{ n \,|\, n \in \A_x \} &=
  \inf\{ \inf\{ n \,|\, \dim U \leq n \} \,|\, \text{$U$ open connected
  neighbourhood of~$x$} \} \\
...
\XXX{customary in topos theory, do not confuse with Kähler stuff}
...
\XXX{remark on the nontriviality/bigness of this sheaf}
...
\XXX{give sheaf-theoretic interpretation as the largest open subset...}
...
statement~$\Box\varphi$ holds, we may assume that indeed~$\varphi$ holds. We
can also write this principle in the following way:
...
the~$\Box$-translation for any modal operator~$\Box$. It will be central to
understand truth of formulas on subspaces from the internal language of the
ambient space.  \XXX{sentence is rubbish}
\XXX{check http://arxiv.org/pdf/1101.5442.pdf for references to cite}
...
Assume that there is an intuitionistic proof of an
...
However, it is \emph{not} clear that the translated formulas~$\varphi^\Box$
and~$(\varphi')^\Box$ are equivalent. This follows only by applying the soundness
lemma (two times, once for each direction). We should stress that to apply this
...
equivalence. The difference might not be apparent at the top level (how should
we know that the equivalence holds if not by a proof?\footnote{Of course,
considering logical incompleteness theorems, this questions is slightly
naive.}), but it becomes evident when considering hypothetical situations: To
prove an implication~$\psi \Rightarrow \chi$, we may give a proof of~$\chi$
under the additional assumption that~$\psi$ holds; however, we may not suppose
that there is a proof of~$\psi$ -- indeed,~$\psi$ might later turn out to be
false. \XXX{is this a good (and correct!) explanation?}
...
\Box\chi$. (In general, only the former implies the latter.) The case
...
\[ \Box\varphi \equiv ((\varphi \Rightarrow \notat{\xi}) \Rightarrow \notat{\xi})
  \Leftrightarrow ((\varphi \Rightarrow \bot) \Rightarrow \bot)
  \Leftrightarrow \neg\neg\varphi. \qedhere \]
\end{proof}
...
maybe do this in the section discussing negneg-sheaves.
...
thus the pulled back formula indeed expresses that~$\O_{X,x}$ is finitely free.
\end{proof}
...
Since we want to verify the~$\Box$-stable statement~``$\neg\neg(x = 0)$'', we
may in fact assume that~$a$ is invertible. Then~$x = 0$ obviously follows.

For the ``if'' direction, let~$x\?\F$ be an arbitrary element; we may assume
...
invertibility of~$a$ implies~$x = 0$. This~$\F$ is quasicoherent, for some
natural number~$n$ it holds that~$a^n x = 0$. Since~$a$ is \notnot invertible,
it is regular \XXX{give reference} and~$a^n$ are regular. So~$x \in \F_\tors$.
\end{proof}
...
trivial with our framework. The second statement follows since
...
\begin{lemma}Let~$X$ be a reduced scheme. Then:
\begin{enumerate}
\item $\O_X$ is~$\neg\neg$-separated.
...
\XXX{is~$\Box$ the largest operator such that~$\O_X$ is~$\Box$-separated?}
...
to~$X$ again (see~\XXX{ref}). Let~$\L$ be a line bundle on~$X$. Since~$\L_\xi \cong
...
any module admits a resolution by (not necessarily finitely) free modules is
...
the zero module if~$f$ is not invertible (since then, it is nilpotent). This is
...
We give the proof below, after first discussing some examples and consequences.
...
\begin{rem}\label{rem:rem:qcoh-in-constructive-mathematics}
The notion of quasicoherence as given by the internal condition of
theorem~\ref{thm:qcoh-sheafchar} is not usually studied in constructive
mathematics. Indeed, outside of the internal universe of a scheme, the notion
...
if for any affine open subset~$U \subseteq X$ and function~$f \in
...
if and only if from the internal perspective, the set~$\F[f^{-1}]$ is a
separated presheaf with respect to the modal operator~$(\speak{$f$ inv.}
\Rightarrow \placeholder)$ for any~$f\?\O_X$; and we will see that in this
...
We now interpret the internal statement~\eqref{eqn:separated} using the
...
\XXX{Remind the reader of abuse of notation like "U" for formulas vs. open
sets.}
...
isomorphism, see discussion at \XXX{ref}). To this end, we can use the composition
...
in~$\Sh(X)$}. An internal locale in a topos~$\E$ is given by an object~$L$ of~$\E$
...
not need to describe explicitly.
\XXX{give reference}
...
over~$X$ and internal topological spaces in~$\Sh(X)$. This is one of the
reasons why locales are better suited for working internally and switching
...
This is already enough to prove the claims: By a general lemma of locale theory,
\XXX{give reference}
...
\XXX{cite Hakim and Gillam ("Localization of ringed spaces")}
...
general, this successor ideal is not unique, the axiom of dependent choice
...
the covering is finite. Since the sheaves~$\I_i$ are of finite type, they
...
remark~\rem{rem:qcoh-in-constructive-mathematics}).
\end{rem}
...
\begin{longtable}{@{}p{5cm}@{\qquad}p{5cm}@{\qquad}p{3cm}@{}}
  ...
  $\Int(X \setminus \supp\F)$ & largest open subset on which~$\F$ is a singleton & remark~\ref{rem:support-sheaf-of-sets} \\
  ...
  $\F$ is locally finitely free & $\F$ is finitely free & proposition~\ref{prop:locally-free} \\
  ...
  $\Int(X \setminus \supp\F)$ & largest open subset on which~$\F = 0$ & proposition~\ref{prop:characterization-support} \\
...
\XXX{consistently use "i" for closed embeddings and "j" for open embeddings}

\XXX{explain use of ``:'' even outside the internal language}
