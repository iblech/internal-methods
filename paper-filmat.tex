...
\bigskip
\dotparagraph{``Any function~$\NN \to \NN$ is the zero function or not.''} More
...
\bigskip
\dotparagraph{``Any function~$\NN \to \NN$ is computable by a Turing machine.''}
...
computing a function~$f : \NN \to \NN$, outputs the description of a Turing
...
and~$S_F$ cannot make direct computational use of it, but it ensures that an
\emph{unbounded search} will not fail: The machine~$S_F$ can simulate~$F$ in
order to compute, in turn, the values~$f(0), f(1), f(2), \ldots$, and stop
...
* XXX formal Church--Turing thesis
