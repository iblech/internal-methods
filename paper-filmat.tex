...
topos theory or category theory, having a diverse readership of philosophers of
...
toposes~\cite{artin-grothendieck-verdier:topos}, toposes are viewed as
...
bridge-building program~\cite{caramello:tst}. Geometrically, the classifying
topos of a geometric theory~$\TT$ can be regarded as the generalized space of
models of~$\TT$; this idea is due to Monique
Hakim~\cite{hakim:relative-schemes}, though she did not cast her discovery in
...
Yet more views on toposes are fruitfully used --
reference~\cite{johnstone:elephants} lists ten more -- but we shall not review
them here.
...
thorough review by the three anonymous referees, whose efforts are much
...
\begin{defn}A \emph{topos} is a category which has all finite
...
term also has other uses.}\end{defn}
...
categories~\cite[Section~VI.10]{moerdijk-maclane:sheaves-logic}.}
...
not be equivalent. This inbalance has mathematical applications and is explored
...
\begin{defn}The \emph{\effective topos}~$\Eff$ is the ex/reg completion (as
in~\cite[Section~3.4]{menni:exact}) of the category of assemblies.\end{defn}
...
Statement~(1) is reminiscent of the fact that no nonstandard model of
...
\marginnote{
  \begin{ocg}[printocg=never]{layername}{layerid1}{on}
    \vspace*{-12em}
    \scalebox{0.2}{
      \animategraphics[loop,autoplay]{60}{zeros-in-families-frames/}{0000}{0188}
    }
    \begin{minipage}{3cm}\scriptsize\noindent
    In the online version of this contribution, a video of the
    continuous family can be seen here.\end{minipage}
  \end{ocg}}
...
definitions~\cite{blechschmidt:generic-freeness}.\footnote{This contribution is
