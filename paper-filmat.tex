...
is a model of~\textsc{zf} then~$L^M$, the constructible sets of~$M$ form a
...
Section~\ref{sect:sheaf-toposes}. For most interesting choices of~$X$, the
axiom of choice and the intermediate value theorem will fail for geometric
reasons in~$\Sh(X)$.
...
geometric theory is a theory whose axioms can be put as \emph{geometric
...
of groups contains the \emph{generic group}. This group has exactly those
properties which are shared by any group whatsoever. It is arguably this group
which we implicitly refer to when we utter the phrase ``Let~$G$ be a group.''. XXX
...
Constructions, given a topos~$\E$: XXX
\begin{enumerate}
\item Given an object~$X$ of~$\E$, the \emph{slice topos}~$\E/X$
contains a \emph{generic element} of~$X$.
\item The topos~$\E[\mathbb{O}]$ contains a \emph{generic object}.
\item There is a ``smallest dense'' subtopos $\Sh_{\neg\neg}(\E)$. This topos
is always boolean, even if~$\E$ is not. For a mathematician who employs
intuitionistic logic on their meta level, XXX
\item XXX ultrapower
\end{enumerate}
...
XXX introduction: the objects of the effective topos are data types and the
morphisms are computable maps.
...
\section{Exploring toposes adapted to synthetic differential geometry}
\label{sect:smooth}
...
The following section presents a topos used in \emph{synthetic differential
...
while the field~$\RR$ of ordinary real numbers doesn't contain infinitesimals
...
don't contain any nilcubic numbers which are not already nilsquare. These are
required in order to extend calculations like~\eqref{eq:derivative} to second
derivatives, as in
\[ (x+\varepsilon)^3 - x^3 = x^3 + 3x^2\varepsilon + \frac{1}{2!} 6x
\varepsilon^2. \]
...
that~$\langle0,0,1\rangle^3 = 0$) and the second by passing
...
the basic~$\RR[\varepsilon]/(\varepsilon)^2$ or even~$\RR$ itself. However,
...
However, this doesn't mean that there actually \emph{is} a nilsquare number
...
\subsection{Well-adapted models} The Zariski topos of~$\RR$ contains
...
any function~$R \to R$ is polynomial. Hence important functions such as the
...
\item The ring~$R$ is coincides with~$i(\RR^1)$, the image of the real line
in~$\E$.
\end{enumerate}
\end{defn}

Is is then the culmination of a long line of research by several authors that
several well-adapted models of synthetic differential geometry exist. By the
conditions imposed by Definition~\ref{defn:well-adapted}, for any such
...
proof, any proof making using of these infinitesimals may mechanically be
...
* Sven(?)
* semantic view vs. syntactic view
* Kripke models
* more pointers to topos literature
