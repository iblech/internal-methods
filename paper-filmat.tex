...
only received training in classical logic. For instance, on first sight it
...
where just as~$x_0$ itself, the numbers~$a$ and~$b$ are quite indetermined.
...
intuitionistic logic on their meta level, non-constructive results by their
...
Toposes are impredicative in the sense that any object of a topos has a
...
all equivalent: If~$\Eff$, any function~$\RR \to \RR$ is continuous
while~$\Set$ abounds with discontinuous functions (at least assuming a
...
amounts to stating that there is a Turing machine which, given a natural
...
A rigorous proof that such a machine~$M$ does not exist reduce its assumed
...
To avert a paradox, we should point out where the classical proof of the
...
over~$X$'', then this observation justifies the slogan that ``ordinary mathematics
...
* Reference for the effective topos
...
* Format examples in Eff in a better way
* toposes are not small (Figure 1)
