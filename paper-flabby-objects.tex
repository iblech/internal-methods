...
\begin{prop}
\begin{enumerate}
\item Assuming the law of excluded middle, divisible abelian groups satisfy
the Baer criterion.
\item Assuming Zorn's lemma, abelian groups which satisfy the Baer criterion
are injective.
\end{enumerate}
\end{prop}
...
\subseteq B$. To verify that~$A_0 = B$, let an element~$x \in B$ be given.
...
\begin{prop}
\begin{enumerate}
\item Any abelian group embeds into a divisible abelian group.
\item Any abelian group embeds into an abelian group satisfying the Baer
condition.
\end{enumerate}
\end{prop}
...
an abelian group which is a sheaf for a modal operator~$\nabla$ such
that~$\nabla(\varphi \vee (\varphi \Rightarrow \nabla\bot))$. Let~$i : \aaa
\hookrightarrow \ZZ$ be the inclusion of an ideal of~$\ZZ$ and let~$f : \aaa
\to A$ be a linear map. Applying the sheafification functor~$(\cdot)^{++}$, we
...
The given map~$f$ factors (uniquely) over the map~$\aaa \to \aaa^{++}$ since~$A$ is a
sheaf. The map~$i^{++}$ is still injective as sheafification is exact. The two
top vertical maps need not be injective, as~$\ZZ$ need not be separated
for~$\nabla$; but this issue XXX
\end{proof}

\begin{prop}Assuming Zorn's lemma:
\begin{enumerate}
\item Any abelian group embeds into an injective abelian group.
\item Any module embeds into an injective module.
\item XXX Grothendieck topos?
\end{enumerate}
\end{prop}
