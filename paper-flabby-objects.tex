...
\begin{proof}Let~$X \in \F$ be a flabby object. Without loss of generality, it
suffices to verify condition~$({\star})$ for the terminal stage~$A = 1 \in \E$.
Let~$k : K \to 1$ be a monomorphism in~$\E$ and let~$x : K \to f_*(X)$ be an arbitrary morphism.
Then~$f^*(k) : f^*(K) \to 1$ is a monomorphism in~$\F$ and~$x^t : f^*(K) \to X$
...
By assumption, the morphism~$f_*(B) \to f_*(1)$ is an epimorphism.
\end{proof}
...
It seems that Harting was not aware that this construction is possible, as she
...
\begin{scholium}For any elementary topos~$\E$:
\begin{enumerate}
\item Any object can be embedded into an internally injective object.
\item The underlying unstructured object of an internally injective module is
...
in~$\E$, we see that the object of preimages of~$s$ is object. Since~$\E$ is
