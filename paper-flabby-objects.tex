...
\begin{abstract}
  ...
  research started by Roswitha Harting, we use this notion to
...
Zorn's lemma is available, then~$X$ possesses a global element (a morphism~$1 \to X$).
\end{prop}
...
(mistakingly) states in~\cite[page~233]{harting:remark} that ``the notions of
injectivity and internal injectivity do not coincide'' for modules.
% XXX "mistakenly"? comparison with Harting's proof (Barr's lemma...)
...
\item The terminal object is flabby. The product of flabby objects if flabby.
...
\item In the ef{}fective topos, the statement ``if~$X$ is flabby, then any map~$X
\to X$ has a fixed point'' holds for ef{}fective sets~$X$. In particular, the
only module in the ef{}fective topos which is flabby and an ef{}fective set is the
trivial module (XXX proof). On the other hand, assuming the law of excluded middle, the
statement ``any module with~$\neg\neg$-stable equality can be embedded into a
flabby module'' holds in the ef{}fective topos. This is because for such a
module~$M$, the canonical morphism~$M \to \Delta(\Gamma(M))$ is a monomorphism,
the set~$\Gamma(M)$ is flabby by virtue of being inhabited, and~$\Delta$
preserves flabby objects by Proposition~\ref{prop:pushforward-of-flabby-objects}.
Here~$(\Delta \dashv \Gamma) : \Set \to \Eff$ is the inclusion of the
