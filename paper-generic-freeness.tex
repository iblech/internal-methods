...
\ldots x_n^{i_n} v_\ell)_{(\ell,i_1,\ldots,i_n) \in J}$ thus generates~$M$ as
...
\begin{figure}
  ...
  have already been removed from the family. If the vector in the red cell will
  be found to be expressible as a linear combination of vectors with smaller
  index (blue cells), it will be removed, along with the vectors in all cells
  to the top and to the right of the red cell. Corners are indicated by the
  small black squares.}
\end{figure}
...
\]
where the index~$j$ ranges over all corners of~$J$, and therefore finitely
