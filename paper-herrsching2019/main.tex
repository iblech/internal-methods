...
Show that any local locale has a canonical point, the \emph{focal point}.
\end{exercise}
...
of~$L|k$ is the classifying locale of the theory which has one nullary relation
symbol~``$\sigma_{xy}$'' for each pair~$\langle x,y \rangle \in L$ and the
following axioms:
\begin{align*}
