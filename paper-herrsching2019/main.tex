...
subset~$I' \subseteq I$ such that~$\top = \bigvee_{i \in I'} U_i$. A
topological space~$Y$ is compact iff its induced locale~$L(Y)$ is.
...
fundamental. A prime ideal is a subset~$\ppp \subseteq A$ such that~$0 \in
\ppp$, $(x \in \ppp) \wedge (y \in \ppp) \Rightarrow (x + y \in \ppp)$, $(x \in
\ppp) \Rightarrow rx \in \ppp$ (for any~$r \in A$),~$1 \not\in \ppp$ and~$(xy
\in \ppp) \Rightarrow (x \in \ppp) \vee (y \in \ppp)$.

Correspondingly, the geometric theory of prime ideals has no sorts; no function
...
and has the prime ideals of~$A$ as its point, but the induced topology on its
...
\begin{enumerate}
  ...
  \item a map~$(\cdot)|^U_V : F(U) \to F(V)$ for each opens~$V \preceq U$
\end{enumerate}
...
Definition~\ref{defn:presheaf}, and also any other definition in this section,
...
\begin{enumerate}
\item $A^\sim$ is a local ring: $\neg(1=0)$, $\forall x,y\?A^\sim_ x + y = 1 \Rightarrow
(\exists z\?A^\sim\_ xz = 1) \vee (\exists z\?A^\sim\_ yz = 1)$.
...
So assume that~$x - y$ is invertible. XXX\end{proof}

\begin{proposition}The sheaf of rings~$A^\sim$ is anonymously
Noetherian.\end{proposition}

\begin{proof}We argue internally. Let~$\aaa \subseteq A^\sim$ be an ideal. XXX
\end{proof}
