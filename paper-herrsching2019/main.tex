...
these situations, the pointfree approach allowed by locales help to give
...
subcovering (a self-contained exposition of this phenomenon is
...
\begin{exercise}{On the embedding of sober topological spaces into locales}%
Verify the statements in Remark~\ref{rem:sober-embed}.
\end{exercise}
...
\end{enumerate}
The category of locales also has small products;
Exercise~\ref{ex:products-of-locales} is devoted to this fact.
\end{exercise}
...
\begin{exercise}{Localic maps into discrete spaces}%
...
recommend the accessible account by Steve Vickers~\ref{vickers:continuity} for a fuller
picture.
...
\item Define the locale of partial surjections~$\NN \twoheadrightarrow \NN^\NN$.
...
Lebesgue measure less than~$\frac{1}{2}$''.
...
\to \RR$ is zero-dimensional.
\end{enumerate}
\end{exercise}

\begin{exercise}{Models of theories as points of the classifying locales}%
...
\begin{exercise}{Product of locales}\label{ex:products-of-locales}%
Let~$X$ and~$X'$ be locales, classifying some propositional geometric
theory~$\TT$ respectively~$\TT'$. Let~$\TT + \TT'$ be the combined theory
...
\begin{exercise}{Coproduct of locales, revisited}%
Let~$X$ and~$X'$ be locales, classifying some propositional geometric
theory~$\TT$ respectively~$\TT'$. Find a theory in terms of~$\TT$
...
\item \emph{Deligne's completeness theorem} states that coherent locales (in
fact, even coherent \emph{toposes}) are spatial. Show that Deligne's
completeness theorem implies that propositional coherent theories
...
denoted~``$\llbracket\varphi\rrbracket$'', such that~$U \models \varphi$.
...
\begin{exercise}{On the failure of the fundamental theorem of algebra}%
...
\[ \Sh(X) \models \exists z\?F\_ z^2 = f. \]
...
\begin{proposition}[Grothendieck's generic freeness lemma, simple version]
...
\item Let~$k$ be a field. Devise a hypothetical notion of
\emph{free~$k$-fields} (where a~$k$-field is a field together with a
homomorphism from~$k$) and show that there are no free~$k$-fields.
\end{enumerate}
\end{exercise}
...
also apllies to this extension, and moreover, this extension in sound with
...
\begin{exercise}{The units of~$A^\sim$}%
...
\begin{exercise}{Identifying the triviality of the spectrum}%
...
Let~$A$ be a ring. Let~$\Spec(A)$ be its spectrum.
\begin{enumerate}
...
is an integral domain such that any finitely generated ideal is locally a
principal ideal.
\begin{enumerate}
\item Let~$A$ be a ring. Show that, if~$A$ is an integral domain then so
is~$A^\sim$. Does the converse hold?
