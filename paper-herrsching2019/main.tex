...
certain ``mirror image'' of~$A$ exists, a sheaf~$A^\sim$ of rings on~$X$. On the one hand,
...
\emph{quasicoherence}) and its its usefulness for constructive algebra
...
\item There is general machinery for free constructions of this kind, and this
machinery is \emph{not} applicable to construct free fields.
\end{enumerate}
...
group homomorphism~$G^\ab \to M$ rendering the diagram
\[ \xymatrix{
...
theory of abelian groups extended by one constant~$x_g$ for each element~$g \in
G$ and the axioms
\begin{align*}
...
factors via a local homomorphism over~$A \to A'$ as indicated in the diagram.
\[ \xymatrix{
...
the canonical morphism~$A \to A_\fff,\,x \mapsto \frac{x}{1}$. Furthermore,
this map is local.
...
X \models phi
  ...
  &\text{iff}\quad \bigvee_{n \in \NN} \exists m \in \NN\_ \exists i_1,\ldots,i_n \in I\_ \exists
  u_1,\ldots,u_n \in A\_ f^m = \sum_{j=1}^n u_j g_{i_j}
