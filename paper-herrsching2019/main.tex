...
certain constructive proofs. The textbook
argument~\cite[Theorem~7.57]{johnstone:topos-theory} establishing the more
powerful version of Barr's theorem requires the axiom of choice, but recently
Michael Rathjen lowered the required metatheory to~\textsc{zf}~\cite[Remark~4.2]{rathjen:barr}.
...
To verify that~(2) implies~(1), we define a semantics by the clauses in
Table~\ref{table:classifying-topos}. The following properties of the semantics
can then be established by induction:
\begin{alphlist}[(d)]
\item XXX % ...; idea: do only the propositional case
\end{alphlist}
\end{proof}
...
\begin{table}
  \tbl{A syntactic model for XXX}{\begin{tabular}{@{}l@{\ \ }c@{\ \ }l@{}}
    $\alpha \models \beta$ &iff& $\TT$ proves~$\alpha \vdash \beta$ \qquad(for~$\beta$ a relation symbol) \\
