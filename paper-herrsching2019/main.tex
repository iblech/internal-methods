...
% typesetting. I personally recommebd using it, but my contribution does not
% depend on it.
...
\begin{table}[t]
  \tbl{The rules of geometric logic}{\begin{minipage}{0.8\textwidth}
    ...
    \textnormal{mixed rules (unnecessary in presence of the double rule for implication)} \\\smallskip
    \vspace{-0.5em}
    \phantom{a}\hfill
...
\paragraph{Extracting constructive content from classical proofs.} Geometric
...
theorem} holds. This theorem comes in two versions, both concerned with
eliminating classical reasoning from proofs of geometric sequents:
\begin{enumerate}
...
geometric logic.
\end{enumerate}
For instance, consider the theory of algebraically closed geometric fields,
that is the theory of rings extended by the axioms
\begin{align*}
  \top &\seq{x:R} (x = 0) \vee (\exists y\?R\_ xy = 1) \\
  \top &\seq{a_0:R,\ldots,a_{n-1}:R} \exists x\?R\_ x^n + a_{n-1}x^{n-1} +
  \cdots + a_1x + a_0 = 0.
\end{align*}
The \emph{Nullstellensatz} is the statement that for any
...
use it in the remainder of this text, we take the opportunity to give a
self-contained proof.

However, the more powerful version is not constructive. This puts us in the
curious situation that we are promised, unconstructively, that there exists
a constructive proof. The textbook argument~\cite[Theorem~7.57]{johnstone:topos-theory} establishing the more powerful
version of Barr's theorem requires
the axiom of choice, but recently Michael Rathjen showed that~\textsc{zf}
suffices~\cite{rathjen:barr}.

\begin{theorem}Let~$\TT$ be a geometric theory. Let~$\sigma$ be a geometric
sequent over the signature of~$\TT$. Then the following statements are
equivalent.
\begin{enumerate}
\item There is a~$\TT$-derivation of~$\sigma$ in geometric logic.
\item There is a~$\TT$-derivation of~$\sigma$ in infinitary intuitionistic logic.
...
with Friedman's trick. We give a sketch of the main thrust of the argument,
omitting lengthy verifications. Let~$\sigma \equiv (\alpha \seq{\vec x} \beta)$.
We introduce the operator~$\nabla$ with~$\nabla\varphi \defeqv
((\varphi \Rightarrow \beta) \Rightarrow \beta)$, where care has to be taken to
avoid capturing the variables~$\vec x$. This operator is a \emph{local
operator} in the sense that for any formula~$\varphi$ in some context~$\vec y$,
infinitary intuitionistic logic proves the sequents
\[
\top \seq{\vec x,\vec y} (\varphi \Rightarrow \nabla\varphi),\
...
Again, care has to be taken to ensure that there is no variable collision with the
variables~$\vec x$. The chosen local operator is special in that
the~$\nabla$-translation of the law of excluded middle,
\[ \top \seq{\vec x,\vec y} \nabla(\varphi^\nabla \vee (\varphi^\nabla
\Rightarrow \nabla\bot)) \]
is intuitionistically provable. Straightforward inductions on the structure of
...
y} \varphi^\nabla$.
\item For any geometric formula~$\varphi$ in some context~$\vec y$, infinitary
intuitionistic logic proves~$\varphi^\nabla \mathrel{\dashv\vdash\!\!\!_{\vec x,\vec y}} \nabla\varphi$.
\item For any derivation of a sequent~$\varphi \seq{\vec y} \psi$ in infinitary
classical logic, there is a derivation of~$\varphi^\nabla \seq{\vec y}
\psi^\nabla$ in infinitary intuitionistic logic.
\end{alphlist}
Combining these ingredients, the given derivation of~$\sigma \equiv (\alpha
\seq{\vec x} \beta)$ in infinitary classical logic can be transformed to give a derivation of
\begin{align*}
  \alpha[\vec x'/\vec x] &\seq{\vec x,\vec x'}
  \nabla(\alpha[\vec x'/\vec x]) \seq{\vec x,\vec x'}
  (\alpha[\vec x'/\vec x])^\nabla \\
  &\seq{\vec x,\vec x'}
  (\beta[\vec x'/\vec x])^\nabla \seq{\vec x,\vec x'}
  \nabla(\beta[\vec x'/\vec x]) \mathrel{\dashv\vdash\!\!\!_{\vec x,\vec x'}}
  ((\beta[\vec x'/\vec x] \Rightarrow \beta) \Rightarrow \beta).
\end{align*}
in infinitary intuitionistic logic. Substituting~$\vec x'$ by~$\vec x$, we obtain
\[ \alpha \seq{\vec x} ((\beta \Rightarrow \beta) \Rightarrow \beta) \seq{\vec
x} \beta. \]

To verify that~(2) implies~(1), XXX
\end{proof}

\begin{table}[ht]
  \tbl{The definition of the~$\nabla$-translation}{\begin{minipage}{\textwidth}\begin{align*}
    (x = y)^\nabla &\defeqv \nabla(x = y) \\
    R(y_1,\ldots,y_m)^\nabla &\defeqv \nabla(R(y_1,\ldots,y_m)) \\
    \top^\nabla &\defeqv \nabla\top \quad \\
    \bot^\nabla &\defeqv \nabla\bot \\
    (\varphi \wedge \psi)^\nabla &\defeqv \varphi^\nabla \wedge \psi^\nabla &
...
and sound with respect to intuitionistic first-order logic (which we fix for
concreteness in Table~\ref{table:first-order-logic}):
...
\makeatother
\begin{table}[t]
  \tbl{The rules of intuitionistic first-order logic}{\begin{minipage}{0.8\textwidth}
    \centering
    \medskip
