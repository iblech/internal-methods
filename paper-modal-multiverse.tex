...
\begin{definition}A topos~$\E$ is \emph{positive} if and only if the geometric
...
theory~$\TT$ to be positive is that xxx.\end{example}


\subsection{Ouvert toposes}

\begin{definition}A topos~$\E$ is \emph{ouvert} if and only if the geometric
morphism~$f : \E \to \Set$ is open, that is~$f^*$ preserves the interpretation of
first-order formulas.\end{definition}

\begin{example}The topos of sheaves over a topological space is always ouvert. More
...
With~\textsc{lem}, every locale is ouvert~\cite{xxx}.\end{example}

\begin{example}The spectrum of a ring~$A$ is ouvert iff every element of~$A$ is
...
of~$\TT$, are inhabited~\cite{xxx}.\end{example}
...
\bibliography{mapaper-modal-multiverse}
