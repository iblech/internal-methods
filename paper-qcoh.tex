...
polynomials~$f$ and~$g$ with coefficients in~$U_\TT$. To say that~$\sigma$ holds for~$U_\TT$ amounts to
...
It is not entirely obvious how to correctly phrase
axiom~\eqref{eq:nullstellensatz0} in first-order logic, an issue
which we will discuss when we restate Theorem~\ref{thm:characterization0} in
the body of this note (Theorem~\ref{thm:characterization}).
...
To verify that~(3) implies~(4), we can mimic the usual proof of that for
coherent sequents, being derivable in geometric logic implies being derivable
in coherent logic, in
...
\end{proof}
% XXX check last paragraph; change proves* to proves.
...
for those special sequents, even~$\TT$ proves~$(\speak{$\sigma$ holds for~$U_\TT$}
...
% XXX justification name quasicoherent
...
% XXX "the internal statement"? be more clear...
...
\begin{enumerate}
\item The sequent~$\sigma$ holds for~$U_\TT$. \smallskip
\item The sequent~$\sigma$ is provable$^\star$ modulo~$\ul{\TT}/U_\TT$ in extended
geometric logic.
\end{enumerate}
\end{thm}
...
* XXX restrict to finitary first-order formulas in the scholium
