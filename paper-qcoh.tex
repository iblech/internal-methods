...
\section{Background on \ldots}

\subsection{the internal language of Grothendieck toposes}

% XXX


\subsection{internal geometric theories}

% XXX, especially the notion of provability and
% provability*

Given an internal theory~$\Sigma$ internal to a Grothendieck topos~$\E$ (or
...
internal theory~$\TT$ over~$\Sigma$, the object of proof trees of~$\TT$.


\subsection{classifying toposes}
...
Writing~$y : \C_\TT \to \Set[\TT]$ for the Yoneda embedding, the \emph{generic
model}~$U_\TT$ of~$\TT$ interprets a sort~$X$ of~$\TT$ as the sheaf~$y\{x\?X\_ \top\}$,
...
symbol~$R \rightarrowtail X_1 \cdots X_n$ by the subobject~$y\{\vec x\_ R(\vec
x)\} \rightarrowtail y\{\vec x\_ \top\}$.
...
\begin{prop}Let~$\alpha$ and~$\varphi$ be geometric formulas in a context~$\vec
...
\item $\{\vec x\_ \alpha\} \models \varphi$.
...
\begin{align*}
  \Spec(A^\sim)(T) &\cong \Hom(yT, \Spec(A^\sim)) =
  \Hom(yT, [A^\sim,U_\TT]_{U_\TT}) \\
  &\cong \Hom(yT \times A^\sim, U_\TT)_{\text{$U_\TT$-algebra homomorphism in second
  argument}} \\
  &\cong \Hom_{U_\TT}(A^\sim, (U_\TT)^{yT})
...
denotes the set of~$U_\TT$-algebra homomorphisms; $(U_\TT)^{yT}$ is the object of morphisms
from~$yT$ to~$U_\TT$; and~$U_\TT|T$ is the functor~$U_\TT(T \times \cdot)$, that is~$S
\mapsto T \otimes S$.

An arbitrary element~$f \in (yA)(T)$, that is an arbitrary~$\TT$-model
...
Let~$A$ be a finitely presented~$\TT$-model. Then the canonical morphism
...
functor~$\Spec(A^\sim)$ coincides with~$yA$. Since by assumption~$A$ is
contained in the site defining~$\Set[\TT]$, the exponential~$(U_\TT)^{yA}$
...
presented. Then the canonical morphism
...
By \emph{geometric higher-order logic} we mean the extension of geometric
logic where we are allowed to form, in addition to the basic sorts supplied by
a given signature: finite limits of sorts; set-indexed colimits of sorts; and
powers of sorts. These derived sorts come with respective term constructors
(tuple formers, coprojections, set comprehension) and the usual rules governing
these constructors.

A \emph{geometric higher-order formula} is a formula built from equality and
...
quantification can be over any sort, including the derived sorts. A
\emph{geometric higher-order sequent} is a sequent of the form~$(\varphi
\seq{\vec x} \psi)$ where~$\varphi$ and~$\psi$ are geometric higher-order
formulas and the sorts of the variables~$\vec x$ may be derived sorts.

The truth of geometric higher-order sequents is in general not preserved under
pullback along geometric morphisms; however for the fragment not containing
powersorts, this is true. In fact, there is a canonical way of associating to
any geometric higher-order sequent~$\sigma$ in this fragment a
family~$(\sigma_i)_i$ of ordinary geometric sequents such that~$\sigma$ is
provable in geometric higher-order logic if and only if all the
sequents~$\sigma_i$ are provable in ordinary geometric logic.
