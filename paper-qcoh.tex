...
of~$\ul{\TT}/U_\TT$ over a stage~$A$. Then there is a covering~$(A_i \to A)_i$
...
of~$\ul{\TT}(A)/U_\TT(A)$. Any function symbol and relation symbol
of~$\ul{\TT}(A)$ occuring in~$\varphi$ is locally given by a symbol in~$\TT$.
...
in~$\varphi$. Then there is a covering~$(\{ \vec y_j\_ \alpha_j \}
\xrightarrow{[\theta_j]} \{ \vec x\_ \alpha \})_j$ such that, restricted to~$\{
\vec y_j\_ \alpha_j \}$, $R$ is given by a symbol~$R_j$ of~$\TT$. To construct
...
\[ A \models \speak{$\ul{\TT}/U_\TT$ proves$^\star$ $\bigl((\exists \vec y_j\_
\theta_j) \seq{\vec x} \bigvee_j \{ \top \,|\, \text{$(\exists \vec y_j\_ \theta_j)$
holds for~$U_\TT$} \}\bigr)$} \]
which in turn follows from Lemma~\ref{lemma:truth-to-provability} exploiting
that~$(\exists \vec y_j\_ \theta_j) \wedge (\exists \vec y_k\_ \theta_k)
\seq{\vec x} \bigvee \{ \top \,|\, j = k \}$.
\end{proof}
...
stage~$A$ and any section~$\sigma$ of the sheaf of
...
semantic side and the axiom scheme~\eqref{eq:nullstellensatz} imply that we may
...
% XXX corollary: function spaces

% XXX prove higher-order characterization?
