...
\begin{frame}{Harnessing formal topology}
  ...
  \visible<3->{
    \raggedright
    For suitable~$E$, $E$-valued points of~$X^\NN$ are for instance:
    \begin{itemize}
...
\begin{frame}{Elephant functions}
  \textbf{Def.} An \hil{elephant function} from~$X$ to~$Y$ consists of
  \begin{itemize}
...
\begin{frame}{Modal language for embracing points}
  ...
  \begin{itemize}
    ...
    (a space such that every open covering of it is inhabited).
...
\begin{frame}{Taming the infinite}
  ...
  \begin{itemize}
    ...
    Notation: ``$\mathrm{Stalls}\,[x_0,\ldots,x_{m-1}]$''
  \end{itemize}
  ...
  \textbf{Thm (classical).} Let~$A$ be a Noetherian\sinf ring. Then~$A[X]$ is Noetherian\ind as well.

  \emph{Proof.} See [Hilbert 1890].
  \emph{This is not mathematics, it is theology!}
  \qed

  \textbf{Thm (constructive).} Let~$A$ be a Noetherian\ind ring. Then~$A[X]$ is Noetherian\ind as well.
  ...
  \emph{Somewhere,} \textsc{lem} holds and crucially~$A$ is still Noetherian\sinf.
...
\begin{frame}{Answering a question by Berardi--Buriola--Schuster}
  ...
  \begin{itemize}
    ...
    sequence~$\alpha : \NN \to X$ has a finite prefix validating~$B$, then so
    does every \emph{arbitrary} infinite sequence everywhere).
  \end{itemize}
