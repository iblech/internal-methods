...
\begin{frame}{Proof mining in approximation theory}
  ...
  \textbf{Theorem.} There is a \hil{unique} best~$L_1$-approximation of~$f$ in~$P_n$.
  \pause
...
\begin{frame}{Backed by logical metatheorems?}
  \hil{Metatheorems} have been and are being developed which \ldots
  \begin{enumerate}
    \item guarantee the \hil{extractability} of suitable numerical information in principle
    (bounds on norms, convergence rates, moduli of uniqueness, rates of
    asymptotic regularity, \ldots),
    \item describe an \hil{algorithm} for carrying out the extraction and
    \item support \hil{modular treatments} of auxiliary lemmas,
  \end{enumerate}
  \bad{\textbf{provided}} the input proof is supplied in a
  certain \hil{formal system}.
  \pause
...
\begin{frame}{Curry--Howard's Rosetta stone}
  ...
  \begin{tabular}{r@{}c@{}l}
    ...
    committing a logical fallacy & \connection{emoji-cross} & committing a type error \\
    \pause
...
\begin{frame}{Three case studies}
  ...
  \begin{columns}[c]
    ...
    \begin{column}{0.85\textwidth}
      ...
      \emph{Proof.} There is a \bad{minimal value}~$f(i)$. Set~$j \defeq i+1$. \qed
    \end{column}
  \end{columns}
  \bigskip
