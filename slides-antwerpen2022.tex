...
\newcommand{\speak}[2]{\textcolor{blue!80}{$#1 \models \text{`$#2$'}$}}
...
\begin{frame}{The universal localization}
  ...
  \begin{itemize}
    \item Let~$S \subseteq A$ be multiplicatively closed. Then there
    is the \hil{localization}
    $A[S^{-1}] \defeq \{ \tfrac{x}{s} \,|\, x \in A, s \in S \}$.
    \pause
    \item\justifying Let~$\ppp \subseteq A$ be a prime filter.
    Then the \hil{stalk}~$A_\ppp \defeq A[\ppp^{-1}]$
    is a \hil{local ring}: If a finite sum of elements is invertible, then so is one
    of the summands.
    \pause
    \item Is there a \hil{universal localization} of~$A$?
  \end{itemize}
  ...
  we could always form the universal localization~$A' \defeq A_{\ppp_0}$!
\end{frame}
...
\begin{frame}{Generic models}
  ...
  \begin{minipage}{0.57\textwidth}
    \centering
    \colorbox{mypurple!30}{Is~$1 + 1 = 0$ in the generic ring?} \\
    \scriptsize \phantom{y}nonclassical truth values\phantom{y}
  \end{minipage}
  \begin{minipage}{0.42\textwidth}
    \centering
    \colorbox{mypurple!50}{The generic ring is a field.}
    \scriptsize mysterious nongeometric sequents
  \end{minipage}
  \bigskip
...
\begin{frame}{Applications of the generic prime filter}
  ...
  \textbf{Proof.} \simplespeak{$M$ is also injective as a matrix over~$A^\sim$.
