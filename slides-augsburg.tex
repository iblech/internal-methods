...
\newcommand{\pointthis}[2]{%
  \tikz[remember picture,baseline]{\node[anchor=base,inner sep=0,outer sep=0]%
    ...
    callout relative pointer={(0.5cm,1.2cm)},fill=blue!20] at ($(#1.north)+(-1.2cm,-1.8cm)$) {#2};}%
}%
...
\frame[t]{\frametitle{The internal language of~$\Sh(X)$}
  ...
  \begin{itemize}
    \item \hil{Locality:} If~$U = \bigcup_i U_i$, then
      $U \models \varphi$ iff $U_i \models
      \varphi$ for all~$i$.
    \item \hil{Soundness:} If~$U \models \varphi$ and $\varphi$
...
\begin{frame}\frametitle{Using the dictionary}
  Let~$X$ be a scheme. Employ its \hil{small Zariski topos}:~$\Sh(X)$.
...
\begin{frame}\frametitle{Transfer principles}
  ...
  \item the~$\O_X$-module~$M^\sim$ in~$\Sh(X)$, where~$X = \Spec A$,
  \end{itemize}
  relate?
  \pause
  \vfill
  ...
  \hil{Answer:} $M^\sim$ inherits those properties of $M$ which are stable
  under localization.
\end{frame}
...
\begin{frame}\frametitle{The curious role of affine open subsets}
  ...
  \begin{align*}
    ...
    \Gamma(U, \E_\mathrm{tors}) &= \Gamma(U, \E)_\mathrm{tors} \\
    \Gamma(U, \K_X) &= \operatorname{Quot} \O_X(U) \\
    &\ \ \vdots
  \end{align*}
  \pause
...
\section{Spreading of properties}
...
\begin{frame}\frametitle{The absolute spectrum, internalized}
  ...
  Monique Hakim's spectrum functor from ringed toposes to locally ringed
  toposes.
\end{frame}
...
\begin{frame}\frametitle{The big Zariski topos}
  ...
  \begin{block}{Basic look and feel}
  \begin{itemize}
    \item For each~$S$-scheme~$X$, its functor of points $\ull{X} =
    \Hom_S(\cdot,X)$ is an object of~$\Zar(S)$. It feels like \hil{the set of
    points} of~$X$.
    \item Internally, $\affl$ (given by $\affl(X) = \Gamma(X,\O_X)$)
    looks like a field:
...
\begin{frame}\frametitle{Some internal constructions}
  ...
  \item Let~$X$ be an~$S$-scheme. The functor of points of $\RelSpec_X
  \Omega^1_{X/S} \to X \to S$ has the internal description
  \[ \Hom(\Delta, \ull{X}), \]
...
\begin{frame}
  \sloganwithoutborder{\hil{Understand notions and statements of algebraic geometry
...
\begin{frame}[label=kripke-joyal]\frametitle{More on the internal language}\justifying
...
\begin{frame}\frametitle{The rules of the Kripke--Joyal semantics}\justifying
...
\note{\begin{itemize}\justifying
  \item The rules are not all arbitrary: Rather, they are very finely concerted
  to make the crucial properties about the internal language (monotonicity,
  locality, soundness with respect to intuitinistic logic) true.

  \item If~$\F$ is an object of~$\Sh(X)$, we write ``$f:\F$'' instead
  of~``$f\in\F$'' to remind us that~$\F$ is not really (externally) a set
  consisting of elements, but that we only pretend this by using the internal
  language.
...
\note{\justifying
  The alternate definition ``$U \models \varphi \vee \psi :\Leftrightarrow
  \text{$U \models \varphi$ or $U \models \psi$}$'' would not be local.
  \par
}
...
\begin{frame}\frametitle{Transfer principles}\justifying
...
\section{The big Zariski topos}

\begin{frame}\frametitle{The big Zariski topos}
  Let~$X$ be a scheme. The \hil{big Zariski topos} is the topos of sheaves
