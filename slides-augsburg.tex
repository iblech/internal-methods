...
\begin{frame}\frametitle{The relative spectrum}
  Let~$X$ be a scheme and~$\O_X \xrightarrow{\varphi} \A$ be an algebra.
  Can we describe~\hil{$\boldsymbol{\RelSpec_X \A}$} internally?

  Desired universal property:
  \[ \Hom_{\LRS/X}(T, \RelSpec_X \A) \cong \Hom_{\mathrm{Alg}(\O_X)}(\A,
  \mu_*\O_T) \]
  for all locally ringed spaces over~$X$.
  \pause
...
\begin{frame}\frametitle{Comparing the little and the big toposes}
  ...
  \begin{align*}
    (\flat \affl)(X \xrightarrow{\mu} S) &= \Gamma(X, \mu^{-1} \O_S) \\
    \affl(X \xrightarrow{\mu} S) &= \Gamma(X, \O_X)
  \end{align*}
  \end{itemize}
\end{frame}
