...
\begin{frame}{Working internally to toposes}
  ...
  \jnote{1}{
    ...
    In particular, we can use the internal language to define when a
    given~$\TT$-structure in~$\E$ is a model -- namely iff it looks like a
    model from the internal point of view.

    The internal language is implemented by the \emph{Kripke--Joyal semantics},
  ...
  \jnote{2}{
    ...
    particular set~$X$'') and dependent types. With these extensions, we can
...
%   is an isomorphism.
  ...
  \jnote{1}{
    ...
    Carsten Butz and Peter Johnstone (\fixedhref{XXX}{Classifying toposes for
    first-order-theories}).
  ...
  \jnote{2}{
    ...
    ones. (This was for instance used by XXX.)

    However, we face some challenges when pursuing these techniques, including
    the following:
  ...
  \jnote{3}{
    ...
    toposes over certain important locales in algebraic geometry -- and
...
\begin{frame}{Affine schemes}
  ...
  \jnote{3}{
    ...
    about~$A$; on the other hand, it enjoys a unique properties rendering it
    simpler than~$A$.
...
\begin{frame}{A systematic source of nongeometricity?}
  ...
  \jnote{2}{
    ...
    the question in a way to exclude the trivial counterexamples given by any
    instance of the law of excluded middle in the language of~$\TT$.) If the
    answer had been positive, this would have given a neat, if somewhat hard to
    check, characterisation of the formulas validated by the generic model.
  }
  ...
  \jnote{3}{
    ...
    would be delighted to be surprised by a future improvement.
  }
