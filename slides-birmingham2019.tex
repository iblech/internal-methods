...
\begin{frame}{Internal conservativity}
  ...
  \begin{multline*}
    \qquad\textit{For any geometric sequent~$\sigma$,} \\
    \textit{if $\Set[\TT] \models \speak{$U_\TT \models \sigma$}$,
    then~$\TT$ proves~$\sigma$}.\qquad
  \end{multline*}
  But~$U_\TT$ \hil{does not believe} that it is conservative in the sense that
  \hil{neither}
  \begin{multline*}
    \qquad\textit{For any geometric sequent~$\sigma$,} \\
    \Set[\TT] \models (\speak{$U_\TT \models \sigma$} \Rightarrow
    \speak{$\TT$ proves~$\sigma$}) \qquad
  \end{multline*}
  \hil{nor}
  \begin{multline*}
    \qquad\Set[\TT] \models \ulcorner\text{For any geometric sequent~$\sigma$,} \\
      \text{if~$U_\TT \models \sigma$ then~$\TT$
      proves~$\sigma$}\urcorner\qquad
  \end{multline*}
  hold.
\end{frame}
...
\begin{frame}{The Nullstellensatz}
  ...
  \[ \text{if~$U_\TT \models \sigma$, then~$\underline{\TT}/U_\TT$
    proves$^\star$~$\sigma$.}
  \]
\end{frame}
