...
\begin{frame}{Exhaustion and extensions}
  ...
  The morphism~$\mathrm{ev}$ below is an isomorphism.
  \begin{equation}\label{eq:hon}\tag{$\star$}
    \mathrm{ev} : \mathrm{FunctFormulas}^\star(\ul{\TT}/U_\TT)/(\dashv\vdash) \longrightarrow P(U_\TT)
  \end{equation}

  \textbf{Theorem 4.} A higher-order formula holds for~$U_\TT$ iff it is
  provable in intuitionistic higher-order logic from the axioms of~$\TT$ and
  the higher-order Nullstellensatz scheme~($\star$).
