...
\begin{frame}{A systematic source of nongeometricity?}
  ...
  \[ \xymatrix{
    \text{$\TT$ proves~$\varphi$\;} \ar@/_1pc/@{=>}[r] &
    \text{\;$\varphi$ holds for~$U_\TT$\;} \ar@/_1pc/@{=>}[r]
    \ar@/_1pc/@{=>}@[red][l]
    |{\SelectTips{cm}{}\textbf{\scalebox{3}{\textcolor{red}{\texttimes}}}}|{} &
    \text{\;$\varphi$ is~$\TT$-redundant}
    \ar@/_1pc/@{=>}@[red][l]
    |{\SelectTips{cm}{}\textbf{\scalebox{3}{\textcolor{red}{\texttimes}}}}|{}
  } \]
\end{frame}
...
\newcommand{\nullstellensatz}{%
  ...
  \begin{quote}\emph{\mbox{For any \explainstub{geometric$^\star$ sequent}{red!20}~$\sigma$ over
...
\begin{frame}{A varying internal theory}
  ...
  \begin{enumerate}
    \justifying\small
    \item for each element~$x : U_\TT$, a constant symbols~$e_x$,
  ...
  \textbf{Remark.} Externalizing the internal classifying
...
\begin{frame}{Example applications}
  ...
  Let~$x \? U_\TT$. Assume~$\neg(x = 0)$. By the Nullstellensatz~$\ul{\TT}/U_\TT$ proves~$(x = 0
  \Rightarrow \bot)$. But this is false in the~$\ul{\TT}/U_\TT$-model~$(U_\TT
  \amalg \{ \heartsuit \})$.
  ...
  Hence it holds in the~$\ul{\TT}/U_\TT$-model~$U_\TT[X]/(f)$. Hence~$g = 0$
