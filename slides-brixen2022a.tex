...
\begin{frame}[c]
  ...
  \begin{tikzpicture}
    \def\R{8pt}
    \node (title) {\vbox{\vspace*{-0.0em}Revisiting \hil{divisible},
    \hil{injective} and \hil{flabby} abelian groups \\ from a \hil{constructive
    point of view}\\[0.8em]}};
    \begin{pgfonlayer}{background}
...
\begin{frame}{Cohomology}
  ...
  \vspace*{-0.5em}
  \mbox{\emph{Basic tool for sheaf cohomology and derived functors:}} \\
  \bad{injective resolutions}
\end{frame}}

\begin{frame}{Integrated development?}
  \begin{block}{}
    \justifying
    \textbf{Prop.}
    There are \hil{irrational} numbers~$x,y$ with~$x^y$ \hil{rational}.
  \end{block}
  \vspace*{-0.3em}
  {\small\emph{Proof.} Either~$\sqrt{2}^{\sqrt{2}}$ is rational or not.
  %\begin{enumerate}
    %\item
    In the first case we are done.
    %\item
    In the second case we set~$x \defeq \sqrt{2}^{\sqrt{2}}$ and~$y \defeq
    \sqrt{2}$. \qed\par}
  %\end{enumerate}

  \pause
  \bad{No effective procedure described by the proof.}
  \pause
  Contrast with:
  \begin{block}{}
    \justifying
    \textbf{Thm.}
    Let~$A \in K^{n \times m}$,
    $b \in K^n$. Then the set $\{ x \in K^m \,|\, Ax = b \}$
    is either empty or of the form~$x_0 +
    \operatorname{span}(v_1,\ldots,v_\ell)$.
  \end{block}
  \vspace*{-0.6em}
  {\small\emph{Proof.} By elementary row transformations, we can \ldots\qed\par}
  \pause

  %\grayline

  \begin{enumerate}
    \item\justifying \hil{Constructively,} we work
    without~\badbox{\textsc{lem}}, \badbox{\textsc{ac}} and
    \badbox{\textsc{zorn}}.
    \item\justifying Constructive proofs always yield \hil{algorithms} and
    \hil{global versions} for continuous families.
    \item\justifying \hil{Concrete} classical results can \hil{always be
    constructivized}: If~\bad{\textsc{zfc}} proves an
    arithmetical~$\Pi^0_2$-statement, so does~\textsc{izf}.
  \end{enumerate}
\end{frame}
...
\begin{frame}{}%Injective and divisible abelian groups}
  ...
  \begin{enumerate}
    ...
    \\\visible<4->{\emph{Proof.} If~$A \cong \ZZ\langle M \rangle/R$, then~$A
...
\begin{frame}{Logical sheaves}
  ...
  {\vspace*{-1.7em}\\\centering\small\it ``continuous~$\NN$-valued functions
  which \\ agree on a dense open agree everywhere''\par}
  \pause
  but not a \hil{$\neg\neg$-sheaf}, which would mean
  \[ \forall S \subseteq \NN\_ \neg\neg(\text{$S$ singleton}) \Rightarrow
    \exists! x \? \NN\_ \neg\neg(x \in S). \]
  {\vspace*{-1.7em}\\\centering\small\it ``continuous~$\NN$-valued functions
  defined on a \\ dense open can be extended to all of the space''\par}
  \pause

  \mbox{\hil{Sheafify}:
  $\NN^+ \defeq \{ S \subseteq \NN \,|\, \neg\neg(\text{$S$ singleton})
  \}/{\sim}$
  with $S \sim T \vcentcolon\Leftrightarrow \neg\neg(S = T)$.}
  ...
  \vspace*{-0.3em}
  \begin{enumerate}
    \item \justifying\textbf{Prop.}
    The group~$(\QQ/\ZZ)^+$ has the Baer property. \vspace*{-0.6em}
    \item \textbf{Lemma.}
    \badbox{\textsc{zorn}} Let~$A$ be a group. Let~$x_0 \in A$. Then
    there is an uncanonical map $i : A \to (\QQ/\ZZ)^+$ such that~\mbox{$i(x_0) = 0 \Rightarrow
    \neg\neg(x_0=0)$.} \vspace*{-1.8em}
    \item \textbf{Thm.}
    Every group canonically maps to a Baer group, and with
    \badbox{\textsc{zorn}}, this map is injective.
  \end{enumerate}
\end{frame}
...
\begin{frame}{Perspectives}
  ...
  \vspace*{-0.5em}
  \hil{\Huge \smiley} \\
  The existence of enough injectives requires only \badbox{\textsc{zorn}}, \\
  not \badbox{\textsc{lem}} or \badbox{\textsc{ac}}, and is hence
  \hil{constructively neutral}.

  Assuming \badbox{\textsc{zorn}} on the metalevel, \\
  it also holds for continuous families.
  \pause

  \bigskip
  \bad{\Huge \frownie} \\
  Still not fully computational. \\
  Cohomology still requires fixing a universe of sets.
  \pause

  \grayline

  \emph{Plan:} \\
  \hil{Resolve issue} by doing without injectives, \\
  inspired by \hil{Emily Riehl}.

  \vspace*{-4.62em}\hspace*{-25.5em}\includegraphics[height=6.5em]{cathtpy-pic}
\end{frame}
\addtocounter{framenumber}{-1}
