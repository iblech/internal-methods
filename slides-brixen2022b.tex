...
\begin{frame}{A topos-theoretic multiverse}
  ...
  \newcommand{\seqbad}{\only<3>{infinite descending sequence}\only<4->{bad set}}
  ...
  \small
    ...
    \pause
    \pause
    \item Given a set~$X$, \emph{proximally} there is a surjection~$\NN
    \twoheadrightarrow X$. \\
...
\begin{frame}{Extracting constructive content}
  \begin{block}{}
    \justifying
    \textbf{Prop.} [Dickson] If~$X$ and~$Y$ are almost-full\textsubscript{ind},
    so is~$X \times Y$.
  \end{block}
  \pause
  ...
  into a function~$i : \NN \to \NN$. \vspace*{-0.2em}\pause
...
Mit LEM hat jede Sequenz α ein Minimum:
Induktion über einen Wert, der vorkommt (zum Beispiel α(0)).

Mit LEM gilt: ∃n. α(n) < x  oder  nicht.
Im ersten Fall fertig nach IV.
Im zweiten Fall:
  Sei n beliebig. Weiß ¬(α(n) < x).
  Also weiß: ¬(α(n) ≤ x) und ¬¬(x ≤ α(n)).
  Mit LEM also x ≤ α(n).

Mit DC bekomme Folge:
i₀ so, dass α(i₀) Minimum.
i₁ so, dass α(i₁) Minimum unter α(i₀+1), α(i₀+2), ...

Betrachte Folge (β(iṉ))ṉ.
Mit LEM finde Minimum: β(ik).
Dann β(ik) ≤ β(i(k+1)) und natürlich auch α(ik) ≤ α(i(k+1)).


x < y  heißt:  x ≤ y und ¬(y ≤ x)
Behauptung: ≤ af und transitiv, dann < wf.

Betrachte generische absteigende Folge.
Da gut, habe x[i] ≤ x[j].
Wegen Transitivität auch x[i] > x[j],
also x[j] < x[i], also x[j] ≤ x[i] und ¬(x[i] ≤ x[j]).
Widerspruch.
