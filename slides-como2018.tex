...
\begin{frame}[c]
  ...
  \hil{How topos theory can help algebra} \\
...
\begin{frame}{Motivating testcases}
  ...
  \jnote{1-3}{The two displayed statements are trivial for fields. It is
  therefore natural to try to reduce the general situation to the field
  situation.}

  \jnote{2-3}{The displayed proofs, which could have been taken from any
  ...
  \href{https://www.ams.org/journals/proc/1988-103-04/S0002-9939-1988-0954974-5/S0002-9939-1988-0954974-5.pdf}{nontrivial
  uses of nontrivial rings} and in the
  \href{https://arxiv.org/abs/1605.04832}{recent textbook by Lombardi and
  ...
  \href{https://rawgit.com/iblech/internal-methods/master/paper-generic-freeness.pdf}{about
  a page}.}
...
\begin{frame}{The internal language of a topos}
  ...
  and dependent types. Shulman's \href{https://arxiv.org/abs/1004.3802}{stack
  semantics} offers what we need. No knowledge of stacks is necessary to enjoy
  his paper. Prior work includes
  \href{https://www.cl.cam.ac.uk/~amp12/papers/polist/polist.pdf}{Polymorphism
  is Set Theoretic, Constructively} by Pitts and
  \href{http://www.phil.cmu.edu/projects/ast/Papers/Awodey-Butz-Simpson-Streicher-APAL-2013.pdf}{Relating
...
\begin{frame}{The internal language of~$\boldsymbol{\Sh(X)}$}
  ...
  && \quad\quad \text{there exists~$s_0 \in F(U_i)$ s.\,th. $U_i \models \varphi(s_0)$} \\
  ...
  \to \G$ be a morphism of sheaves on~$X$. Then:
  \begin{align*}
...
\begin{frame}{Internalizing parameter-dependence}
  ...
  translations shown on the slide (the corner quotes ``$\speak{\ldots}$''
  indicate that translation into formal language is left to the reader), this
  ...
  \href{https://raw.githubusercontent.com/iblech/internal-methods/master/images/zeros-in-families.mp4}{this
  video shows}.
...
\begin{frame}{The little Zariski topos}
  ...
  rings which are not trivial yet don't have any prime ideals or prime filters.
...
\begin{frame}{First steps in the little Zariski topos}
  ...
  \jnote{1-3}{The generic prime filter~$\fff_0$ can be described in explicit
  terms. For ring elements~$f$ and~$s$, $D(f) \models (s \in \fff_0)$ iff~$f
  \in \sqrt{(s)}$.}

  \jnote{2-3}{Our description of~$M^\sim$ reveals a precise sense in
  ...
  satisfied if the law of excluded middle is available. In an intuitionistic
  context, it's still ``morally satisfied''. Details are in Section~12.9 of
  \href{https://rawgit.com/iblech/internal-methods/master/notes.pdf}{these
  notes}.}

  \jnote{3}{As a first approximation, the module~$M^\sim$ can be thought of as
  a reification of all the stalks of~$M$ as a single object. The metatheorem
  makes this precise and also shows the limits of this view: It is only correct
  for geometric sequents. When considering nongeometric sequents, phenomena
  appear which are unique to~$M^\sim$ in the sense that they are in general not
  shared by~$M$, its stalks or its quotients.}
