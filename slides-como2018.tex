...
\begin{frame}{Synthetic algebraic geometry}
  ...
  merits of the functorial approach. Two disadvantages of the approach using
  locally ringed spaces is that the underlying topological spaces don't
  actually parametrize ``honest'', ``geometric'' points, but irreducible closed
  subsets; and that they don't work well in a constructive setting. (For this,
  they would have to be replaced by locally ringed locales.)

  Given a functor~$F : \mathrm{Ring} \to \Set$, we imagine~$F(A)$ to be the set
  of~``$A$-valued points'' of the hypothetical scheme described by~$F$, the set
  of ``points with coordinates in~$A$''. These sets are geometrically important.

  The functorial approach is more economical, philosophically rewarding, and
  works constructively. However, typically only field-valued points are easy
  ...
  \end{array} \]
  It are these more general kinds of points which impart a meaningful sense of
  ...
  functors~$\Ring \to \Set$ is a topos (the big Zariski topos of~$\Spec(\ZZ)$)
  and that we can therefore employ its internal language. This language takes
  care of managing stages behind the scenes. For instance, projective~$n$-space
