...
\begin{frame}[c]
  ...
  \jnote{1}{\textbf{This talk in a nutshell:}

  There is a certain ring, the \emph{generic ring}, which is special in that
  it is \emph{conservative}: It has exactly those properties which are shared
  by any ring whatsoever. One could argue that it is this ring which we're
  implicitly referring to when we utter the phrase ``Let~$R$ be a ring.''.

  A surprising observation is that the generic ring is a field. This is not a
  contradiction because conservativity only pertains to properties which can be
  put in a certain syntactic form, dubbed \emph{geometric sequents}, and the
  field condition is not of that form.

  As a consequence, when verifying a geometric sequent for all rings, we may
  freely assume the field condition: Our proof will apply to the generic ring,
  because this ring happens to be a field, and its conclusion will spread to
  any ring whatsoever thanks to its syntactic form.

  An analogous story can be told for the generic group, the generic vector
  space and quite generally for the generic model of any \emph{geometric
  theory}. It is an interesting task to discover properties of such
  generic models. This talk presents a source of such properties, the
  \emph{topos-theoretic Nullstellensatz}, which is
  universal in the sense that any such property is a logical consequence of the
  Nullstellensatz.}
\end{frame}}

{\usebackgroundtemplate{\begin{minipage}{\paperwidth}\hspace*{-1.7cm}\includegraphics[height=\paperheight,trim=0 0 5.1cm 0,clip]{arthurs-seat-4}\end{minipage}}
\begin{frame}[plain]%
  \vspace*{\paperheight}\vspace*{-2.35cm}\hspace*{-1cm}%
  \includegraphics[width=\paperwidth/4]{arthurs-seat-7}%
  \includegraphics[width=\paperwidth/4]{arthurs-seat-0}%
  \includegraphics[width=\paperwidth/4]{arthurs-seat-5}%
  \includegraphics[width=\paperwidth/4]{arthurs-seat-9}%
  \jnote{1}{This talk owes a special debt to Alexander Oldenziel, who
  repeatedly suggested that the topic of this talk should be studied.

  The photos on the slide are of \emph{Arthur's Seat}, a beautiful extinct
  volcano in Edinburgh where parts of these slides have been created. It gives
  an excellent view of the city and is very much recommended for a hiking tour.
  }%
\end{frame}}
\addtocounter{framenumber}{-1}
...
\begin{frame}{The mystery of nongeometric sequents}
  \jnote{1}{A \emph{geometric sequent} is a syntactic expression of the
  ...
  \emph{geometric formulas}. Such a sequent is read as ``in the
  context~$x_1\?X_1,\ldots,x_n\?X_n$, $\varphi$ entails $\psi$''.
  Geometric formulas are built from atomic propositions (equality and the
  available relation symbols) using the connectives~$\top$, $\bot$, $\wedge$, $\bigvee$
  ...
  any given topos. For instance, a ring in the usual sense is a model of the
  theory of rings in the topos~$\Set$. The structure sheaf~$\O_X$ of a scheme~$X$ is a
  model in the topos~$\Sh(X)$ of set-valued sheaves over~$X$.
  ...
  they provide us with new reduction strategies (proofs by ``without loss of
  generality'').}
...
\begin{frame}{Construction of the generic model}
  ...
  \begin{enumerate}
    \item objects: \explainstub{formulas-in-contexts}{red!20} $\{ x_1\?X_1, \ldots, x_n\?X_n\_ \varphi \}$
  ...
  topos}~$\Set[\TT]$. The generic model interprets a sort~$X$ by the sheaf~$\yon\{x\?X\_\top\}$.
  ...
  empty context modulo provable equality) is a model of~$\TT$. While the
  term algebra does enjoy some nice categorical properties, it is in general
  ...
  sheaves over~$\C_\TT$, called the \emph{classifying topos} of~$\TT$.}

  \jnote{2}{The notation in the definition of the objects of~$\C_\TT$ is meant
  to be suggestive, but should not be taken literally. The objects are
  syntactic expressions consisting of a context and a geometric formula in that
  context.
...
\begin{frame}{Working internally to toposes}
  ...
  Write~``$\Sh(\C) \models \varphi$'' for~$U \models \varphi$ for all~$U$.
  \only<1>{\footnotesize
  ...
  \only<2>{
    ...
    \begin{columns}[t]
      \begin{column}{0.45\textwidth}\justifying
        \textbf{Example.}
        $\Set \models \varphi$ if and only if $\varphi$ is true.
        \bigskip

        \textbf{Theorem.}
        If~$\Sh(\C) \models \varphi$ and if $\varphi$ intuitionistically
        entails~$\psi$, then also~$\Sh(\C) \models \psi$.
      \end{column}

      \begin{column}{0.45\textwidth}
        \href{https://arxiv.org/abs/1004.3802}{\includegraphics[width=\textwidth,align=t]{mike-shulman-stack-semantics}}
        \vspace*{1.5em}
      \end{column}
    \end{columns}
  }
  ...
  \jnote{2}{
    We can actually \emph{do mathematics} internally to a topos because the
    Kripke--Joyal semantics is sound with respect to intuitionistic logic:
    If~$\E \models \varphi$ and if~$\varphi$ intuitionistically entails a
    further formula~$\psi$, then~$\E \models \psi$.
...
%   is an isomorphism.
  ...
  \jnote{1}{
    ...
    sort and no function symbols, relations symbols or axioms, is
...
\begin{frame}{An application in commutative algebra}
  ...
  The \hil{generic local localisation}~$A^\sim$ of~$A$ validates:
  ...
  This observation unlocks a short and conceptual proof of Grothendieck's
  generic freeness lemma in algebraic geometry.

  \jnote{1}{A \emph{local localisation} of a ring~$A$ is a
  localisation~$A[S^{-1}]$ which is a local ring. The classifying topos of
  local localisations of~$A$ is none other than the topos of sheaves over the
  spectrum of~$A$, as studied in algebraic geometry; and the generic model is
  also known as the structure sheaf~$\O_{\operatorname{Spec}(A)}$.
  ...
  are constructively valid for this Noetherian condition.)
...
\begin{frame}{A systematic source of nongeometricity?}
  \jnote{1-2}{
    ...
    are modelled by plain old sets, morphisms of schemes by plain old maps
  ...
  \jnote{2}{
    ...
    Thierry remarked at 6WFTop, this is even if one takes care to phrase
...
\begin{frame}{A topos-theoretic Nullstellensatz}
  \jnote{1-2}{
    ...
    The asterisk in \emph{geometric$^\star$ sequent}
    indicate that any infinities used to index
    disjunctions have to come from the base topos. This restriction is an important
    subtlety, though not vital to this talk. If~$\TT$ is a coherent
    theory, then we can restrict to coherent sequents and thereby vanquish the
    subtlety.
  }

  \jnote{2}{
    The algebraic Nullstellensatz states that, in some cases, algebraic truths
    are witnessed by explicit \emph{algebraic certificates} -- syntactic
    ...
    they can be expressed as geometric sequents, and algebraic certificates
    are replaced by \emph{logical certificates}: proofs.
  }

  \jnote{3}{
    While, as stated on slide 1/10, the generic model~$U_\TT$ is a
    ...
    \mbox{For any geometric sequent~$\sigma$, if~$\Set[\TT] \models
    \speak{$\sigma$ holds for~$U_\TT$}$, then $\TT$ proves~$\sigma$.}

    The following is false in general: \\
    \mbox{$\Set[\TT] \models \speak{For any geometric$^\star$ sequent~$\sigma$,
    if~$\sigma$ holds for~$U_\TT$, then $\ul{\TT}$ proves~$\sigma$}$.}

    The following is true: \\
    \mbox{$\Set[\TT] \models \speak{For any geometric$^\star$ sequent~$\sigma$,
    if~$\sigma$ holds for~$U_\TT$, then $\ul{\TT}/U_\TT$ proves~$\sigma$}$.}
  }
...
\begin{frame}{A varying internal theory}
  ...
  \begin{enumerate}
    ...
    \item for each function symbol~$f$ and~$n$-tuple~$(x_1,\ldots,x_n) : (U_\TT)^n$ the axiom
    $(\top \vdash f(e_{x_1},\ldots,e_{x_n}) = e_{f(x_1,\ldots,x_n)})$,
    \item for each relation symbol~$R$ and~$n$-tuple~$(x_1,\ldots,x_n) : (U_\TT)^n$ such
  ...
  \jnote{1}{
    ...
    Constructing internally to~$\Set[\TT]$, where~$\ul{\TT}/U_\TT$ looks just like an
...
\begin{frame}{Revisiting the test cases}
  ...
  %  \{\heartsuit\}$.
    Let~$x,y \? U_\TT$. We want to verify~$\neg\neg(x = y)$. Hence assume that~$\neg(x = y)$.
    By the Nullstellensatz~$\ul{\TT}/U_\TT$ proves~$\sigma \defeq (e_x = e_y \vdash \bot)$.
    Thus~$\sigma$ holds in any~$\ul{\TT}/U_\TT$-model.
    But~$\sigma$ is false in the~$\ul{\TT}/U_\TT$-model~$M \defeq U_\TT/(x \sim
    y)$. Hence~$\bot$.
  }
  ...
  \jnote{1}{
    This slide gives an example how to use the Nullstellensatz to deduce
    ...
    Often when using the Nullstellensatz, we go from a (perhaps assumed) truth
    of~$U_\TT$ via provability to another model~$M$
    of~$\ul{\TT}/U_\TT$. That is, we use provability as a (one-way)
    \emph{bridge}.
  }
\end{frame}
...
\begin{frame}{Exhaustion and extensions}
  ...
  \jnote{1}{
    ...
    exhaustive. It is possible, but not entirely obvious, how to formulate the
    Nullstellensatz as an (infinitary) first-order formula.
    ...
    Theorems~3 indicates that the Nullstellensatz can be generalised to the
  ...
  \jnote{2}{
    ...
    Theorem~3 rests on a relativisation of Olivia Caramello's completeness theorem,
  ...
  \jnote{3}{
    ...
    \begin{itemize}\justifying
      ...
      territory from a logical point of view. The talk by Matthias Hutzler will
      present new results in that direction.
    \end{itemize}
  }
\end{frame}}
