...
\begin{frame}{A case study in double negation}
  ...
  \textbf{Thm.} Let~$X$ be an inhabited set of natural numbers.
  \begin{enumerate}
...
\begin{frame}
  ...
  \emph{* higher-order proofs of first-order statements using the assumption \\
  that a given (perhaps uncountable) set is countable}
\end{frame}

\begin{frame}{A quantifier for finite approximations}
  By a \hil{finite approximation} to an infinite sequence~$\alpha : \NN \to X$,
  we mean a \hil{finite prefix} of~$\alpha$. Notation:

  \begin{itemize}
    \item empty list $[]$
    \item extension $[x_1,\ldots,x_n] ::^r x_{n+1} = [x_1,\ldots,x_n,x_{n+1}]$
    \item we write~``$\tau \preceq \sigma$'' iff~$\tau$ is
  a refinement of~$\sigma$: $[x_1,\ldots,x_n,x_{n+1},x_{n+m}] \preceq
  [x_1,\ldots,x_n]$
  \end{itemize}
  \justifying
  ...
  \[
    ...
    \qquad
    \infer[{\text{\scriptsize$(\sigma \in X^*)$}}]{\nabla (\tau \preceq \sigma)\_ P(\tau)}{\forall(x \in X)\_\ \nabla(\tau
    \preceq (\sigma ::^r x))\_ P(\tau)}
  \]
  \[
    \infer[{\text{\scriptsize$(\sigma \in X^*, a \in X)$}}]{\nabla(\tau \preceq \sigma)\_ P(\tau)}{\forall(\tau \preceq \sigma)\_\
