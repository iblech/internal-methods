...
\begin{frame}{The double-negation embedding}
  ...
  \begin{enumerate}
    ...
    \item \textbf{Thm.} Every infinite binary sequence
    contains repeated terms.
    \medskip
  ...
  \begin{enumerate}
    \item If~\badbox{\textsc{lem}}, then~$X$ contains a minimal element.
    \item<5-> If~$X$ is \hil{detachable}, then~$X$ contains a minimal element.
    \item<7-> It is \notnot the case that~$X$ contains a minimal element.
  \end{enumerate}
