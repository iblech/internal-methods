...
\begin{frame}{The double-negation embedding}
  ...
  \textbf{Thm.} Every infinite binary sequence~$\alpha : \NN \to \mathbbm{2}$
  contains repeating entries.

  {\emph{Proof.} By the \bad{infinite box principle}, which follows
  from~\badbox{\textsc{lem}}, the sequence contains infinitely many zeros or
  infinitely many ones. In either case the claim follows.\par}

  How to extract \good{constructive proofs} from these
  \bad{classical proofs}?
  \pause

  \grayline

  \textbf{Thm.} Let~$X$ be an inhabited set of natural numbers. Then~$X$
  contains a minimal element.

  {\emph{Proof.} By~\badbox{\textsc{lem}}, \ldots\par}
\end{frame}
