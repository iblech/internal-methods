...
\begin{frame}{}
  ...
  \jnote{6-}{
    ...
    Chuangjie Xu's course, particularly in employing variants of the
    ...
    All invocations of the transfinite such as by the law of excluded middle or
...
\begin{frame}[plain]
  ...
  \jnote{1}{
    ...
    It is astonishing what she started and which long way we have come!
...
\begin{frame}{Exploring the realizability model}
  ...
  \jnote{10}{
    ...
    realizability model it looks quite different that from the point of view of
...
\begin{frame}{A case study in double negation}
  ...
  \jnote{3-4}{
    ...
    extraction of constructive content is the form of the asserted claims than
    the form of the used auxiliary lemmas.
  }
  ...
  \jnote{5-}{
    ...
    (together with corresponding realizers).
  }
...
\begin{frame}{The double-negation embedding}
  \jnote{1-3}{
    The starting point of the double-negation embedding is that, while the law
    of excluded middle is not available in intuitionistic logic, the double
...
%    This observation unlocks the following technique to transcribe a classical
%    proof of a statement to an intuitionistic (and even minimal logic) proof of
%    its translation. Most steps in the classical proof will also be acceptable
%    in intuitionistic logic. We simply mimic these. When the classical proof
%    appeals to~\textsc{lem}, we instead use the appropriate doubly negated
%    instance. We
%
%    The difference between intuitionistic and classical logic is concentrated
%    in their treatment of disjunction and existential quantification.
%    Classically, to verify a disjunction~$\varphi \vee \psi$, it suffices to
%    establish~$\neg(\neg\varphi \wedge \neg\psi)$--the intuitionistically
%    weaker claim that it is not the case that neither~$\varphi$ nor~$\psi$
%    hold; similarly, to verify an existential statement~$\exists x\?X\_
%    \varphi(x)$, it suffices to establish~$\neg\forall x\?X\_
%    \neg\varphi(x)$---the claim that nonexistence is impossible.
  }
...
\begin{frame}{Barr's theorem / Friedman's trick / A-translation}
  ...
  \begin{enumerate}
    ...
    \item (Assuming \badbox{\textsc{ac}}) $\sigma$ is provable from~$\Gamma$ in
...
\begin{frame}{A quantifier for finite approximations}
  ...
  \begin{itemize}
    ...
    \item refinement relation: $[x_1,\ldots,x_n,x_{n+1},x_{n+m}] \preceq [x_1,\ldots,x_n]$
