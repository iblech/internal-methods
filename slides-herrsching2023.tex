...
{\usebackgroundtemplate{\begin{minipage}{\paperwidth}\centering\includegraphics[height=\paperheight]{multiverse-faded-169}\end{minipage}}
\begin{frame}[c]
  \jnote{1}{These are the slides for a three-part lecture series for the autumn school
...
\begin{frame}{A brief timeline}
  \jnote{4}{
    ...
    \[ \text{if $\textsc{zfc} \vdash \varphi$, then $\textsc{zf} \vdash
    \varphi^L$}. \]
    Specializing to~$\varphi \defeqv \bot$ we obtain: If~\textsc{zfc} is
    inconsistent, then so is~\textsc{zf}. The axiom of choice does not
    introduce new inconsistencies.

    In modern semantic language: While the axiom of choice might fail in the
    base universe~$V$ (= the class of all sets), it always holds in~$L$.
  ...
  \jnote{5}{
    ...
    Just as a given ring~$R$ or group can be extended in various ways, to include
    ...
    similar such extensions, but of universes (traditionally set-sized models
    of~\textsc{zfc}, but also class-sized models of~\textsc{zfc}, or models
    of intuitionistic set theories, or models of type theories, or even models
    of arithmetic) instead of rings.

    In semantic language, from a high level the idea is the following: Whether
