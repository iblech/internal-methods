...
\begin{frame}{The eventually monad}
  ...
  We then define~``\hil{$P \mid \sigma$}'' (``$P$ bars~$\sigma$'') inductively by the clauses:
  \begin{enumerate}
...
\begin{frame}{The $\nabla$-translation}
  \small
  For bounded first-order formulas over the (large) first-order signature which has
  \begin{enumerate}
    \scriptsize
  ...
  \end{tabbing}
  \small
  Finally, we say that~$\varphi$ ``holds in~$V^\nabla$'' iff for all~$\sigma
