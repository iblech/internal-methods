...
\begin{frame}{How should this notion be formalized in HoTT?}
  ...
  \textbf{Def.} (classically) A preorder~$X$ is \hil{well} iff every sequence~$\NN \to X$ is good.

  \textbf{Examples.} (classically)\ $(\NN,{\leq}),\ \ X \times Y,\ \ X^*,\ \ \mathrm{Tree}(X).$
  \medskip
  \pause

  \subhead{\emph{A naive formalization attempt}}

  \textbf{Def.} $\mathsf{Well}(X,{\leq}) \defeq
  ...
  \begin{itemize}
    \item[\cmark] short
    \item[\cmark] sweet
    \item[\cmark] immediate
    \columnbreak
    \item[\xmark] philosophically strenuous  %: needless reference of the transfinite
    \item[\xmark] not practical  %: cannot verify closure properties
    \item[\xmark] not faithful(?)  %: missing out on some sequences
  \end{itemize}
  \end{multicols}
\end{frame}}
