...
\begin{frame}{Sequences depending on an environment}
  ...
  Then the type~$\NN \to X$ misses the \hil{generic enumeration} of~$X$.
  \medskip
  \pause

  \emph{Idea:} Approximate a (fictitious) surjection~$\NN \twoheadrightarrow X$
  ...
  Formally, we have the type~$\nabla\underline{X}$ of stage-dependent values of~$X$:
\end{frame}
