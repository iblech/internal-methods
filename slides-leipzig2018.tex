...
\begin{frame}{Toposes are \ldots}
  ...
  isn't a true topology in the usual sense. Putting the étale topology on a
  ...
  numbers, functions, manifolds -- but the properties they enjoy differ
  slightly from topos to topos.
  ...
  structure, which is crucial for a rounded understanding. However, the
  displayed definition is trimmed for minimality and therefore doesn't mention
  it. A more comprehensive definition is: A \emph{topos} is a locally cartesian
...
\begin{frame}[fragile]{A glimpse of the toposophic landscape}
  ...
  We'll get to know some of these better during the remaining course of the
  talk.

  For any topological space~$X$, there is a topos~$\Sh(X)$ of \emph{sheaves
