...
\begin{frame}{Maximal ideals}
  ...
  \only<4-7>{\bigskip\par\centering\scalebox{0.9}{\begin{tikzpicture}
    \node (0) at (0,1) {$(0) = \{0\}$};
    \node (1) at (0,5) {$(1) = \ZZ$};
    \node (2) at (-2,4) {$(2)$};
    \node [right of=2] (3) {$(3)$};
  ...
  \begin{enumerate}
    ...
    \item In the general case: \bad{No}, but we \good{do have} the first-order
    consequences of the existence of a maximal ideal.
  \end{enumerate}
\end{frame}
