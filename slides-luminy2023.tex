...
\xyoption{rotate}
\usepackage{tikz}
\usetikzlibrary{calc,shapes,shapes.callouts,shapes.arrows,patterns,fit,backgrounds,decorations.pathmorphing,positioning,svg.path,fadings}
...
\begin{tikzfadingfrompicture}[name=fade bottom]
  \shade[top color=transparent!0,
    bottom color=transparent!50] (0,0) rectangle (2,2);
      ...
      \begin{minipage}{6cm}
        \begin{columns}
          \begin{column}{0.15\textwidth}
            \hspace*{1.2em}\color{white}\dbend
          \end{column}
...
\begin{frame}{Computational content from classical proofs}
  ...
  \emph{Can we extract a constructive proof that well implies well-founded?}
\end{frame}
...
\begin{frame}{Maximal ideals}
  ...
  \begin{enumerate}
    ...
    {\footnotesize\begin{align*}
      \mmm_0 &= \{ 0 \}, &
      \mmm_{n+1} &= \mmm_n + (\underbrace{\{ x \in A \,|\, x = x_n \wedge
    ...
    \item In the general case: \bad{No}, but the \emph{first-order
...
\begin{frame}{Questions}
  \begin{enumerate}
    \item \!Why was the inductive revolution so powerful?
    \bigskip
...
\begin{frame}{Constructive forcing (= Grothendieck toposes)}
  ...
  {\centering\footnotesize\begin{tabular}{lll}
    \toprule
    preorder~$L$ & coverings of an element~$\sigma \in L$ & filters of~$L$ \\
    \midrule
    $X^*$ & $\{ \sigma x \,|\, x \in X \}$ & maps~$\NN \to X$ \\
    $X^*$ & $\{ \sigma x \,|\, x \in X \}$,\ \
    $\{ \sigma\tau \,|\, \tau \in X^*, a \in \sigma\tau \}$ for each~$a \in X$ & surjections~$\NN \twoheadrightarrow X$ \\
    f.g. ideals & --- & ideals \\
    f.g. ideals & $\{ \sigma+(a), \sigma+(b) \}$ for each~$ab \in \sigma$,\ \
    $\{\}$ if~$1 \in \sigma$ & prime ideals \\
    opens & $\mathcal{U}$ such that~$\sigma = \bigcup \mathcal{U}$ & points \\
    $\{\star\}$ & $\{ \star \,|\, \varphi \} \cup \{ \star \,|\, \neg\varphi \}$ &
...
\begin{frame}{The modal multiverse}
  ...
  \hil{$\blacktriangleright$}
  For \hil{positive open} extensions, they are equivalent for bounded first-order formulas. \\
  ...
  \small
  \begin{multicols}{2}
    For every set~$X$, \emph{proximally} \\
...
\begin{frame}{The multiverse perspective}
  \begin{enumerate}
    \item \!\emph{Why was the inductive revolution so powerful?}
    ...
    -- \textsc{dc} holds \emph{proximally}.
  \end{enumerate}
\end{frame}
