...
\begin{frame}{Infinite data}
  \[ 7,
    \quad \astikznodecircled{x0}{white}{4}\!,
    \quad \only<1-2>{\astikznodecircled{t1}{white}{3}}\only<3->{\astikznodecircled{t1}{mypurple}{3}}\!,
    \quad \only<1>{\ldots}\pause \astikznodecircled{x1}{white}{1}\!,
    \quad \only<2>{\ldots}\pause \astikznodecircled{t2}{mypurple}{8}\!,
    \quad \only<3>{\ldots} \visible<4->{\astikznodecircled{x2}{white}{2}\!,}
    \quad \only<4->{\ldots} \]
  ...
  \textbf{Thm.} Every infinite sequence~$\alpha : \NN \to \NN$ is \hil{good} in that
  there exist numbers~$i < j$ such that~$\alpha(i) \leq \alpha(j)$.
  \pause

  \mbox{\emph{Proof.} \emph{(offensive?)} By~\badbox{\textsc{lem}}, there is a
  minimum~$\alpha(i)$.
  Set~$j \defeq i + 1$.\par}
  \pause
  \bigskip

  \textbf{Def.} A preorder~$X$ is \hil{well} iff every infinite sequence~$\NN \to X$ is good.
  
  \textbf{Examples.} $(\NN,{\leq}),\ \
  \color{white}\only<7->{\color{red!90}}\underbrace{\color{black}X \times Y,\ \ X^*,\ \ \mathrm{Tree}(X)}_{\text{\visible<7->{\bad{only classically}}}}$.
  \pause
  \pause
  \bigskip
  ...
  \centering
  \textbf{Moral:} \\
  \small{\it Don't quantify over points of spaces which might not have enough.}
\end{frame}
...
\begin{frame}
  \vspace*{\paperheight}\vspace*{-3.4em}\centering
