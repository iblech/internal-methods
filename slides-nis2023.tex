...
\begin{frame}{Not in this talk}
  ...
  ``What if there are not enough points?''

  \vspace*{0.2cm}

  \centering
  \includegraphics[width=0.3\textwidth]{confusion}
\end{frame}
...
\begin{frame}{Algebraic geometry in a nutshell}
  Turn \hil{algebras} into \hil{spaces}, and \hil{glue} those
  spaces.
  \bigskip

  \subhead{Examples}
  \begin{enumerate}
    \item $\Spec(R[X_1,\ldots,X_n]) = \AA^n$.
    \item $\Spec(R[X,Y]/(Y-X^2)) = \text{standard parabola}$.
    \item Gluing
...
\begin{frame}{Transfinite methods}
  ...
  despite \ldots
  \begin{enumerate}
    \item concrete subject matter (in part)
  ...
  \pause
  \subhead{Thesis}
  Algebraic geometry can alternatively be developed from just the three postulates of a
  \hil{quasicoherent} \hil{local ring} with \hil{Zariski-choice}.
\end{frame}}

\begin{frame}{A simple spectrum construction}
