...
\begin{frame}{Summary}
  ...
  reasoning, this difference is immaterial. Any intuitionistic theorem about
  rings applies to~$A^\sim$ as if~$A$ were a proper ring.
...
\begin{frame}{Motivating the semantics}
  ...
  construction makes sense even if~$f$ is a zerodivisor, in which
...
\begin{frame}[fragile]{A universal property}
  ...
  homomorphim~$A \to R$ to a local ring uniquely factors over a local ring
...
\begin{frame}
  ...
  model~$A^\sim$.
  \begin{itemize}
    \item It has the pleasant property that it is a field.
