...
\newcommand\ytl[2]{%
  ...
  \parbox[c]{8.4cm}{\vspace{7pt}\raggedright#2\\[7pt]}%
  \\[-2.0pt]}
\begin{frame}{A brief timeline}
  \vspace*{-1.0em}
  \parbox[b]{4.5em}{\hfill\phantom{x}}\makebox[0pt][c]{\phantom{b}}{\color{mypurple}\vrule}\\[-3.5pt]
...
\begin{frame}{Parametrized mathematics}
  ...
  \begin{enumerate}
    ...
    over~$A[f^{-1}]$, \hil{then~$1 = 0$ in~$A$}.}''\only<4>{\\\ }
  \end{enumerate}
  \end{changemargin}
\end{frame}
...
\begin{frame}{The multiverse of parametrized mathematics}
  ...
  \begin{changemargin}{-1.2em}{-1.2em}
    ...
    \begin{enumerate}
      ...
      \smallskip
      \item For every inhabited set~$X$, \emph{proximally} there is a
