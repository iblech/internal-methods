...
\frame[t]{\frametitle{The little Zariski topos}
  ...
  Then $A \to \O_{\Spec(A)}$ is the universal localisation.

  \begin{block}{Definition}The \hil{little Zariski topos} of~$A$ is the
  category~$\Sh(\Spec(A))$ of set-valued sheaves on the \hil{spectrum} of~$A$.\end{block}

  The topology on~$\Spec(A)$ is generated by open subsets~$D(f)$ for~$f \in A$
  such that~$\Spec(A) = \bigcup_i D(f_i)$ if and only if~$1 = \sum_i g_i f_i$
  for some~$g_i$.
}

% Wieso hat LocRing --> Ring keinen Linksadjungierten?
% Sollte also nicht Limiten bewahren.

% Recall the definition of a local ring and a local homomorphism!
...
\section[Little Zariski]{The little Zariski topos of a ring}

\begin{frame}{XXX}
\end{frame}

% O_{Spec(A)} as the reification of all stalks
% O_{Spec(A)} integral domain iff all stalks integral domains
% Transfer theorem

% *Constructed* to yield a universal localisation (mention classifying topos),
% *observed* to fulfil further properties.
% Give Mulvey/Tierney citation, state background (quasicoherence)
% also neg neg (f = 0) ==> f = 0

% Anonymous Noetherian condition
% Prove it
