...
\begin{frame}{The little Zariski topos}
  ...
  with~$A^\sim(D(f)) = A[f^{-1}]$.

  \begin{block}{Motto}The structure sheaf~$A^\sim$ is a reification of
...
\begin{frame}\frametitle{Generic freeness}
  ...
  \hil{Proof.} Internally in~$\Sh(\Spec(A))$,
  \begin{enumerate}
    \item $B^\sim$ and~$M^\sim$ are \hil{not not} free
    over~$A^\sim$
    \item $A^\sim \to B^\sim$ is \hil{not not} of finite presentation,
    and
    \item $M^\sim$ is \hil{not not} finitely presented as a module
    over~$B^\sim$.
  \end{enumerate}
\end{frame}
...
% Think about "share all first-order properties"
