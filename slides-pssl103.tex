...
\newcommand{\intex}[3]{#1 \quad #2\par #3\medskip}
...
\begin{frame}{Nongeometric sequents}
  ...
  \begin{varblock}{11cm}{}  %Fundamental theorem about classifying toposes}
    For a \hil{geometric sequent}~$\varphi \seq \psi$, the following
    are equivalent:
    \begin{enumerate}
      \item The theory~$\TT$ \hil{proves}~$\varphi \seq \psi$. \\[-1em]
      \item $\forall \vec x\_ (\varphi \Rightarrow \psi)$ holds
