...
\begin{frame}{Transfinite methods in algebra?}
  ...
  \only<1>{\centering\scalebox{0.8}{$\begin{pmatrix}
  ...
  \centering\includegraphics[width=0.3\textwidth]{generic-freeness}
\end{frame}
...
\begin{frame}{The sheaf model as a local lens}
  ...
  \begin{tabular}{p{0.31\textwidth}p{0.75\textwidth}}
    ...
    \speak{$\varphi \vee \psi$ on~$D(f)$} &
      there is a partition~$f^n = fg_1{+}\cdots{+}fg_m$ s. th.
      for each~$i$, \phantom{lolololl} \phantom{lololo}\speak{$\varphi$ on~$D(fg_i)$} or \speak{$\psi$ on~$D(fg_i)$} \\
    \speak{$\varphi \Rightarrow \psi$ on~$D(f)$} &
      for all~$g \in A$, \speak{$\varphi$ on~$D(fg)$} implies~\speak{$\psi$ on~$D(fg)$} \\
    \speak{$(\forall x\?A^\sim\_ \varphi(x))$ on~$D(f)$} &
      for all~$g \in A$ and~$x \in A[g^{-1}]$, \speak{$\varphi(x)$ on~$D(fg)$} \\
    \speak{$\bot$ on~$D(f)$} & $f$ is nilpotent
  \end{tabular}
  \bigskip
...
\begin{frame}{Revisiting the test cases}
  ...
  \textbf{Proof.} \speak{On~$D(1)$, $M$ is also injective as a matrix over~$A^\sim$.
  This is a contradiction by basic linear
  algebra.} Thus~\speak{$\bot$ on~$D(1)$}. This amounts to~$1 = 0$ in~$A$.
  \bigskip
  \pause
