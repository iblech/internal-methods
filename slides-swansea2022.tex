...
\begin{frame}[c]
  ...
  \begin{tikzpicture}
    \def\R{8pt}
    \node (title) {\vbox{\vspace*{-0.5em} \textcolor{white}{Maximal ideals in
    (countable) rings, constructively}}};
    \begin{pgfonlayer}{background}
...
\begin{frame}{Transfinite methods in algebra}
  ...
  \bigskip
  \centering
  \emph{-- abstract proofs as blueprints for concrete ones --}
\end{frame}
...
\begin{tikzpicture}[every annotation/.style = {draw, font = \large}]
    \path[
      mindmap,
      concept color=black!00,
      %every node/.style={concept},
      root/.style={concept color=black!00, font=\large\bfseries,text width=2em},
      level 1 concept/.append style={sibling angle=72,text width=9.0em, level distance=8em,inner sep=0pt}
    ]
    node[root] {
    } [counterclockwise from=180]
      child[concept color=blue!00] {
      }
      child[concept color=blue!00] {
        node {\includegraphics[width=2cm]{calabi-yau} \mbox{commutative algebra}}
      }
      child[concept color=blue!00] {
        node {\includegraphics[width=2cm]{multiverse} \mbox{traveling} the \mbox{mathematical multiverse}}
      }
      child[concept color=blue!00] {
      };
  \end{tikzpicture}
...
- philosophical:
  - inconsistent ⇒ 1 = 0
  - maximal ideals as convenient fictions
  - reification of dynamical algebra
- concrete:
  - surjective
  - Suslin
