...
\begin{frame}{Transfinite methods in algebra}
  ...
  \pause\justify%
  \textbf{Proof.}
  \bad{Assume not.} \visible<4->{Then there is~a \bad{maximal ideal} $\mmm$.}
  \visible<6->{The matrix is surjective over the field~$A/\mmm$. This is a contradiction to
  basic linear algebra. \qed}

  \only<5-6>{\par\centering\scalebox{0.9}{\begin{tikzpicture}
    \node (0) at (0,1) {$(0) = \{0\}$};
    \node (1) at (0,5) {$(1) = \ZZ$};
    \node (2) at (-2,4) {$(2)$};
    \node [right of=2] (3) {$(3)$};
