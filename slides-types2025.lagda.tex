...
\addtocounter{framenumber}{-1}
\begin{frame}\frametitle{A classical logic fairy tale}
  ...
  \jnote{1}{
    ...
    \begin{changemargin}{-1.5em}{-1.5em}
      ...
      \begin{multicols}{2}
        ...
        queen of the country and of all Möbius strips called for her royal
        philosopher.
  ...
  \jnote{2}{
    Adapted from
    \href{http://blog.ezyang.com/2013/04/a-classical-logic-fairy-tale/}{Edward
    Yang's blog}.

    The idea conveyed in the fairy tale is at the core of the
    \emph{constructive} proof of $\neg\neg(A \vee \neg A)$. While the law of
    excluded middle (\textsc{lem}) is not available in constructive
    mathematics, up to a double negation every instance of it is.

    This observation gives rise to the double negation translation, a
    transformation of (statements and) proofs in classical logic to proofs in
    intuitionistic logic, at the only expense of introducing double negations;
    by following this up with Friedman's trick with the nontrivial exit
    continuation (``Baby Barr Theorem''), we can in many cases even get rid of
    these double negations in a second step.

    Getting rid of applications of the law of excluded middle is in many
    contexts (an amazing mind-boggingly wonder at first, but then) a routine
    matter---see \href{https://rt.quasicoherent.io/}{here} for more details and
...
\begin{frame}{A case study in Hilbert's program}
  ...
  \jnote{3}{
    ...
    shortly to be renamed~``well\sinf'', these results are
    \href{https://lets-play-agda.quasicoherent.io/Padova2025.Explorations.Forcing.Orthogonal.html}{inherently
    classical} (even the weaker claim ``the product of two streamless sets is
    streamless'' admits countermodels).

    (In Higman's lemma, the set~$X^*$ of finite lists of elements of~$X$ is
    equipped with the following ordering: We have~$(x_0 \dblcolon \ldots
    \dblcolon x_{n-1} \dblcolon [\,]) \leq (y_0 \dblcolon \ldots \dblcolon
    y_{m-1} \dblcolon [\,])$ iff there is an increasing injection~$f : \{
    0,\ldots,n-1 \} \to \{ 0,\ldots,m-1 \}$ such that~$x_i \leq y_{f(i)}$ for all~$i < n$.
    Kruskal's theorem is the result behind the celebrated enormous number
    $\mathsf{TREE}(3)$.)
  }

  \jnote{7}{
    Thanks to work by Thierry Coquand, Daniel Fridlender and Monika
    Seisenberger, a constructive substitute is available, the notion well\ind.
    In classical mathematics (where bar induction is available), this notion is
    equivalent to well\sinf.

    The predicate~$\mathsf{Good}$ of finite lists~$\sigma = (x_0 \dblcolon
    \ldots \dblcolon x_{n-1} \dblcolon [\,])$ appearing in the definition
    of~$G$ expresses that some earlier term is at most some later term,
    i.\,e.\@ that there merely exist~$i < j$ such that~$x_i \leq x_j$.

    The operator $\dblcolon^r$ is adding an element at the end.
  }
  ...
  \[
    ...
    \qquad
    \infer{\mathsf{later}\,f : G\,\sigma}{f : (x : X) \to G\,(\sigma \dblcolon^r x)}
  \]
  \vspace*{-1.3em}
...
\begin{frame}{Missing functions in the type of all functions?}
  \jnote{1}{
    The double negation translation technique fails to constructivize the two
    proofs presented on the slide.

    The reason is that, after applying the translation, the ``$I$-extracted
    subsequence'' appearing in the proof on the left will no longer actually be
    a function~$\NN \to X$. Instead, it will be a partially defined function
    with the property that each number~$n$ does \emph{not~not} belong to the
    domain. However, the assumption that~$X$ is well\sinf{} does not apply to
    these kinds of generalized sequences.

    In contrast, the notion well\ind{} is much stronger. If~$X$ is well\ind,
    then not only will every actual sequence~$\NN \to X$ be good, but (in a
    suitable sense) so will every up-to-$\neg\neg$-partially-defined sequence,
    every multivalued sequence and many more kinds of generalized sequences.
  }
...
\begin{frame}{Where do many cherished inductive definitions come from?}
  \begin{changemargin}{8.0em}{-0.5em}
    ...
    \begin{itemize}
      ...
      \item Force a \hil{generic sequence} $\NN \to X$.
            \fixedhref{https://lets-play-agda.quasicoherent.io/Padova2025.Explorations.Forcing.Cohen.html}{\formalized}
      ...
      \item Force a \hil{generic prime ideal} of a given ring.
            \fixedhref{https://iblech.github.io/constructive-maximal-ideals/Forcing.Prime.html}{\formalized}
    \end{itemize}
    \pause
    ...
    \begin{itemize}
      ...
      is good.
      \item A set is Noetherian iff the generic sequence~$\NN \to X$
      has two repeated terms.
      ...
      prime ideal.
      \fixedhref{https://iblech.github.io/constructive-maximal-ideals/Forcing.Prime.html\#corollary}{\formalized}
    \end{itemize}
    \pause
  ...
  \jnote{1}{
    The technique for enlarging the mathematical universe is called
    \emph{forcing} and was originally pioneered by Paul Cohen. Set theorists
    use forcing to construct new models of set theory from given ones, in order
    to explore the range of set-theoretic possibility. For instance, by forcing
    we can construct models of~\textsc{zfc} validating the continuum hypothesis
    and also models which falsify it.
    ...
    of every instance of~\bad{\textsc{lem}} is an intuitionistic tautology.
  ...
  \jnote{2}{
    Forcing the existence of a new function~$\NN \to X$ is also called
    \emph{Cohen forcing}. Forcing the existence of a new surjection is called
    \emph{Lévy collapse}. Forcing the existence of a prime ideal does not have
    an established name, perhaps it should be called \emph{Zariski forcing}; it
    has been explored by Thierry Coquand and others of the constructive algebra
    crew.

    The multiversal yoga allows us to reason with the good notion well\ind{}
    using the simple language of well\sinf, and similarly with several other
    notions.
  }

  \jnote{3}{
    For more on the mystery of nongeometric sequents, see
    \href{https://www.speicherleck.de/iblech/stuff/slides-birmingham2019.pdf}{this
    set of slides}.
  }
\end{frame}}
...
\begin{frame}{A modal language for harnessing the multiverse}
  ...
  \jnote{3}{
    ...
    so in a suitable topos (a version of the little Zariski topos of the ring).
  }
...
\begin{frame}{The plain combinatorics of toposes}
  \jnote{3-}{
    Proofs (about the base universe) are (pure) programs; proofs about a forcing
    extension are effectful programs unfolding in the~$\nabla$ monad.

    The $\nabla$ monad is a refinement of the (monotonic) state monad.
  }
...
L = List X
    ...
    gadget as assertions about its
    approximations~(\fixedhref{https://iblech.github.io/constructive-maximal-ideals/Forcing.Semantics.html}{\formalized}).
    ...
    \pause
    \item Be prepared to \hil{evolve approximations}
    (\fixedhref{https://iblech.github.io/constructive-maximal-ideals/Forcing.Base.html}{\formalized}).
...
\begin{frame}{Formal metatheory}
  \jnote{2}{
    \href{https://msp.cis.strath.ac.uk/types2025/slides/TYPES2025-slides57.pdf}{Exciting
    ongoing work on split type theory} by Martin Baillon, Assia Mahboubi and
    Pierre-Marie Pédrot!
  }
...
\begin{frame}{Increasing subsequences as convenient fictions}
  \jnote{1}{
    On this slide~$\nabla$ is the monad described on slide 5,
    and~$\nabla^\nnearrow$ is the variant where lists are only allowed to grow
    in a monotonically increasing manner; so~$\nabla$ is the monad for the
    generic sequence~$\NN \to X$, while~$\nabla^\nnearrow$ is the monad for the
    generic increasing sequence~$\NN \to X$.

    The displayed multiversal proof gives a positive answer to a question posed
    by Stefano Berardi, Gabriele Buriola and Peter Schuster, see
    \href{https://www.speicherleck.de/iblech/stuff/slides-abmv2024.pdf\#page=16}{this
    set of slides}.

    At the end of the day, this proof yields a concrete algorithm (see
    \href{https://www.speicherleck.de/iblech/stuff/dickson/implicational.pl}{here}
    for manually-extracted Perl code). Given an infinite sequence, this
    algorithm seeks, in a certain systematic but not exhaustive manner, a
    sufficiently long monotonic subsequence, making judicious use of
    backtracking. This search terminates, though this fact is not obvious just
    by looking at the algorithm.
  }
  ...
  \begin{block}{Constructive reimagination}
    ...
    then~$\nabla\,B\,[\,]$.
    \fixedhref{https://www.speicherleck.de/iblech/stuff/dickson//Dickson.html}{\formalized}
  \end{block}
  \vspace*{-0.4em}
...
\begin{frame}{Maximal ideals as convenient fictions}
  ...
  \emph{Proximally,} there is a \good{maximal
  ideal}~$\mmm$~(\fixedhref{https://iblech.github.io/constructive-maximal-ideals/index.html\#§4.§4-4}{\formalized}).
  ...
  \fixedhref{https://iblech.github.io/constructive-maximal-ideals/index.html\#§4.TestCase}{\formalized}).}
  Write~$M =
...
\begin{frame}{Stabilization as convenient fiction}
  A commutative ring~$A$ with unit is \ldots
  \begin{itemize}
    \item Noetherian\sinf{} iff for every sequence~$x_0,x_1,\ldots$ of ring elements,
    there is a number~$n : \NN$ such that~$x_n,x_{n+1},x_{n+2},\ldots \in
    (x_0,\ldots,x_{n-1})$.
    \item Noetherian\ensuremath{_\text{RS}} iff for every sequence~$x_0,x_1,\ldots$ of ring elements,
    there is a number~$n : \NN$ such that~$x_n \in
    (x_0,\ldots,x_{n-1})$.
    \item Noetherian\ind{} iff a certain inductively defined condition holds
    \fixedhref{https://dl.acm.org/doi/10.5555/646538.696017}{[Coquand–Persson]}.
  \end{itemize}

  We then have:
  \begin{align*}
    \mathsf{Noetherian\ind}(A) &\Longleftrightarrow
    \necessary\mathsf{Noetherian}_\text{RS}(A) \\
    &\Longleftrightarrow
    \necessary\bigl(\badbox{\textsc{lem}} \Rightarrow
    \mathsf{Noetherian}\sinf(A)\bigr) \\
    &\Longleftrightarrow
    \necessary\possible\mathsf{Noetherian}\sinf(A).
  \end{align*}

  With this observation, from their classical counterparts, we can extract
  constructive proofs of:
  \begin{enumerate}
    \item Hilbert's basis theorem: If $A$ is Noetherian\ind, then so is $A[X]$.
    \item Locality: If~$1 = f_1+\ldots+f_n$ and if each
    ring~$A[f_i^{-1}]$ is Noetherian\ind, then so is~$A$. \\
    (As a consequence, $A$ is Noetherian\ind
    iff algebraic geometry's~$\mathcal{O}_{\mathrm{Spec}(A)}$ is.)
  \end{enumerate}
\end{frame}
...
\begin{frame}[plain]
  \jnote{1}{
    \href{https://lets-play-agda.quasicoherent.io/}{Let's play Agda}, an
    interactive course on Agda, complementing the
    \href{https://agdapad.quasicoherent.io/}{Agdapad}.

    \href{https://www.uni-augsburg.de/en/fakultaet/mntf/math/prof/geom/workshop-aimath/}{AI
    Transforms Maths Research}, a workshop in Augsburg.

    \href{https://conferences.cirm-math.fr/3377.html}{Synthetic mathematics}, a
    conference in Luminy.
  }
