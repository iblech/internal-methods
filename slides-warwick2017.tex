...
\begin{frame}\frametitle{Generic freeness}
  ...
  If $R$ is reduced ($x^n = 0 \Rightarrow x = 0$), there is a \hil{dense open subset} $U \subseteq \Spec R$ such that for any $f \in R$ with~$D(f)
  \subseteq U$,
  \begin{enumerate}
...
\begin{frame}\frametitle{Synthetic constructions}
  ...
  \text{set of $\affl$-rational points of $R$}.$} \\[1em]
...
\begin{frame}{Synthetic formulation of properties}
  \begin{itemize}
  \item \ \\[-1.2em]\mbox{$\RelSpec R = \Hom_{\mathrm{Alg}(\affl)}(R, \affl) =
  \text{set of $\affl$-rational points of $R$}.$}
