...
%  \end{itemize}
  ...
  \begin{itemize}
    ...
    \item Understanding generic freeness
...
\subsection[Generic freeness]{Generic freeness}
...
\begin{frame}\frametitle{Generic freeness}
  ...
  If $R$ is reduced ($x^n = 0 \Rightarrow x = 0$), there
...
\begin{frame}\frametitle{Synthetic constructions}
  ...
  \begin{itemize}
    \item $\RelSpec \affl[X,Y,Z]/(X^n+Y^n-Z^n) \cong {}$ \\ \qquad\qquad $\{ (x,y,z) \in (\affl)^3 \,|\, x^n+y^n-z^n = 0 \}$ \medskip
...
\begin{frame}{Synthetic formulation of properties}
  ...
  there exist $f_1,\ldots,f_n \in \affl$ such that
  $x \in U \Longleftrightarrow \exists i\_ f_i \neq 0$.

  \item Open subsets are $\neg\neg$-stable:
  $\neg\neg(x \in U) \Longrightarrow x \in U$.

  \item If $\gamma : \Delta \to X$ is tangent vector with $\gamma(0) \in U$,
