...
\begin{frame}\frametitle{Synthetic algebraic geometry}
  ...
  \begin{itemize}
    ...
    \begin{multline*}
      \text{set of prime ideals of~$\ZZ[X,Y,Z]/(X^2+Y^2-Z^2)$} + {} \\
...
\begin{frame}\frametitle{Some internal constructions}
  \begin{itemize}
  \item $\mathbb{P}^n_S \defeq \{ (x_0,\ldots,x_n) : (\affl)^{n+1} \,|\,
    x_0 \neq 0 \vee \cdots \vee x_n \neq 0 \}/(\affl)^\times.$

  \item $\RelSpec_S A \defeq \Hom_{\mathrm{Alg}(\affl)}(A, \affl).$

  \item $TX = \Hom(\Delta, \ull{X})$,
