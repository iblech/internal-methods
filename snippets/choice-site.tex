...
an~$S$-scheme to its functor of points, is fully faithful. However,
since~$\Sch/S$ is not essentially small, forming the sheaf topos is not
possible in plain Zermelo--Fraenkel set theory.
...
to~$\Sh(\Sch/S)$, as long as one keeps in mind that~$\Sh(\Sch/S)$ might
not contain a subobject classifier.
...
but the restricted Yoneda functor~$\Sch_\U/S \to \Zar(S)$ is.
...
fields, and others. The Stacks Project then defines~$\Zar(S)$
as~$\Sh(\Sch_M/S)$, where~$\Sch_M/S$ is the small category of~$S$-schemes
in~$M$, or equivalently as~$\Sh(\Aff_M/S)$. This approach has the advantage
that one doesn't have to assume the existence of a Grothendieck universe.
...
A disadvantage of employing the sites with the presentation condition is that
the Yoneda functor is only fully faithful when restricted to~$(\Sch/S)_\lfp$.
For instance, in the case~$S = \Spec(\ZZ)$, the schemes~$\Spec(\QQ)$ and the
empty scheme have isomorphic functors of points, whereby~$\Spec(\QQ)$ looks
like the empty set from the internal point of view.\footnote{XXX explicit proof}

In the following, we try to encompass XXX point out XXX
