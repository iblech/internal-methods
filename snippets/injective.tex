...
\begin{rem}The object~$\P_{\leq 1}(\F)$ of subsingletons of $\F$ can be
...
refined to an honest element of~$\F$.\end{rem}
...
\begin{lemma}A sheaf of set or a sheaf of modules is injective if and only if it
...
enters. But the sheaf topos of~$X$ doesn't remember this set. For instance, let~$X$
be an inhabited indiscrete topological space. Then~$\Sh(X)$ is equivalent
to~$\Set$.
\end{rem}
