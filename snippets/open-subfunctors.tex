...
such that for any object~$(T' \xra{f}{\to} T \to S)$ of the site used to
...
if~$x \not\in U$, we can pick~$J = (0)$ (such that the quotient~$\affl/J$ is
...
of~$1$ is zero: If~$\Spec(\affl[f^{-1}])$ is inhabited, $f$ is invertible and
...
\begin{rem}In view of the previous remark, one cannot hope to simplify the
condition in Proposition~\ref{prop:characterization-open-subfunctor} to read as
