...
\begin{prop}In~$\Zar(S)$, open truth values are~$\neg\neg$-stable: If~$U
\subseteq 1$ is an open truth value, and if~$U$ does \notnot hold, then~$U$
actually holds.\end{prop}

\begin{proof}Since~$U$ is an open truth value, there exists an ideal~$J
...
Since~$\affl/J$ is quasicoherent, the canonical homomorphism
\[ \affl/J \longrightarrow XXX \]
XXX
\end{proof}
